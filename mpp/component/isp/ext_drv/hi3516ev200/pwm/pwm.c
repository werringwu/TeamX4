...
#define HI_IO_PWMI_ADDRESS(x)  ((void __iomem *)((unsigned char __iomem *)reg_pwmI_base_va + ((x)-(PWMI_ADRESS_BASE))))
...
#endif
{
    ...
    switch (cmd) {
        case PWM_CMD_WRITE: {
            ...
            if (read_user_linear_space_valid((unsigned char *)(HI_UINTPTR_T)arg, sizeof(PWM_DATA_S)) == 0) {
                osal_printk("pwm address is invalid!\n");
                return -EFAULT;
            }

            ret = copy_from_user(&stPwmData, (PWM_DATA_S __user *)(HI_UINTPTR_T)arg, sizeof(PWM_DATA_S));
            if (ret) {
                return -EFAULT;
            }
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,0,0)
    if (reg_pwmI_base_va) {
        osal_iounmap((HI_VOID *)reg_pwmI_base_va);
        reg_pwmI_base_va = HI_NULL;
    }
#else
    reg_pwmI_base_va = NULL;
#endif
