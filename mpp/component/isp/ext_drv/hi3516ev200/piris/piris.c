...
#define HI_IO_PIRISI_ADDRESS(dev, x)  ((HI_UINTPTR_T)reg_pirisI_base_va[dev] + ((x)-(PIRISI_ADRESS_BASE)))

#define PIRIS_CFG_REG0(dev)        HI_IO_PIRISI_ADDRESS(dev, PIRISI_ADRESS_BASE + 0x03C0)
...
#endif
{
...
#else
    long int dev = (long int)(HI_UINTPTR_T)(file->private_data);
#endif
    PIRIS_DEV *pstPiris;
    ...
    switch (cmd) {
        case PIRIS_SET_FD:
#ifdef __HuaweiLite__
            if (read_user_linear_space_valid((HI_U8 *)arg, sizeof(int)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_SET_ACT_ARGS:
            ...
            ret = copy_from_user(&PirisPos, (int __user *)(HI_UINTPTR_T)arg, sizeof(int));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_SET_ORGIN:
            if (read_user_linear_space_valid((HI_U8 *)(HI_UINTPTR_T)arg, sizeof(PIRIS_DATA_S)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_from_user(&piris_data, (PIRIS_DATA_S __user *)(HI_UINTPTR_T)arg, sizeof(PIRIS_DATA_S));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_GET_STATUS:
            ...
            if (write_user_linear_space_valid((HI_U8 *)(HI_UINTPTR_T)arg, sizeof(PIRIS_STATUS_E)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_to_user((PIRIS_STATUS_E __user *)(HI_UINTPTR_T)arg, &PirisStatus, sizeof(PIRIS_STATUS_E));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
...
#endif
{
    ...
    PIRIS_DEV *pstPiris = (PIRIS_DEV *)(HI_UINTPTR_T)arg;
...
static int hi_piris_isp_register(int dev)
{
    ...
    if ((FUNC_ENTRY(ISP_EXPORT_FUNC_S, HI_ID_ISP) != NULL) && CKFN_ISP_RegisterPirisCallBack()) {
...
#endif
{
    ...
    for (i = 0; i < PIRIS_MAX_DEV_NUM; i++) {
        ...
        p_piris_dev[i]->timer.data = (unsigned long)(HI_UINTPTR_T)p_piris_dev[i];
