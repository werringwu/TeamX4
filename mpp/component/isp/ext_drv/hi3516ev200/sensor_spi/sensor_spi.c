...
#ifndef __HuaweiLite__
struct spi_master *hi_master[SPI_MAX_NUM];
struct spi_device *hi_spi[SPI_MAX_NUM][2];
...
static int __init sensor_spi_dev_init(void)
{
    ...
    ISP_BUS_CALLBACK_S stBusCb = {0};
    stBusCb.pfnISPWriteSSPData = hi_ssp_write;
    if ((FUNC_ENTRY(ISP_EXPORT_FUNC_S, HI_ID_ISP) != NULL) && (CKFN_ISP_RegisterBusCallBack())) {
        for (i = 0; i < ISP_MAX_DEV; i++) {
            CALL_ISP_RegisterBusCallBack(i, ISP_BUS_TYPE_SSP, &stBusCb);
        }
    } else {
