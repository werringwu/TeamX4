...
HI_S32 SAMPLE_AWB_IspRegsInit(HI_S32 s32Handle)
{
    ...
    pstRawStatAttr->bStatCfgUpdate  = HI_TRUE;
...
HI_S32 SAMPLE_AWB_Init(HI_S32 s32Handle, const ISP_AWB_PARAM_S *pstAwbParam, ISP_AWB_RESULT_S *pstAwbResult)
{
    HI_S32 s32Ret;
    VI_PIPE ViPipe;
    SAMPLE_AWB_CTX_S *pstAwbCtx = HI_NULL;
    ...
    if (s32Ret != HI_SUCCESS) {
        AWB_TRACE(HI_DBG_ERR, "Awb lib(%d) vreg init failed!\n", s32Handle);
        return s32Ret;
    }
                      ...
                      ISP_AWB_RESULT_S *pstAwbResult, HI_S32 s32Rsv)
{
    ...
    if (pstAwbCtx->u32FrameCnt % 2 == 0) {
        ...
        hi_memcpy(&pstAwbCtx->stAwbInfo, sizeof(ISP_AWB_INFO_S), pstAwbInfo, sizeof(ISP_AWB_INFO_S));
    ...
    /* record result */
    hi_memcpy(pstAwbResult, sizeof(ISP_AWB_RESULT_S), &pstAwbCtx->stAwbResult, sizeof(ISP_AWB_RESULT_S));
...
HI_S32 SAMPLE_AWB_Exit(HI_S32 s32Handle)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        AWB_TRACE(HI_DBG_ERR, "Awb lib(%d) vreg exit failed!\n", s32Handle);
        return s32Ret;
    }
...
HI_S32 HI_MPI_AWB_Register(VI_PIPE ViPipe, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_AWBLibRegCallBack(ViPipe, pstAwbLib, &stRegister);
    if (s32Ret != HI_SUCCESS) {
        AWB_TRACE(HI_DBG_ERR, "Hi_awb register failed!\n");
    }
...
HI_S32 HI_MPI_AWB_UnRegister(VI_PIPE ViPipe, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_AWBLibUnRegCallBack(ViPipe, pstAwbLib);
    if (s32Ret != HI_SUCCESS) {
        AWB_TRACE(HI_DBG_ERR, "Hi_awb unregister failed!\n");
    }
                                    ...
                                    AWB_SENSOR_REGISTER_S *pstRegister)
{
    ...
    hi_memcpy(&pstAwbCtx->stSnsRegister, sizeof(AWB_SENSOR_REGISTER_S), pstRegister, sizeof(AWB_SENSOR_REGISTER_S));
    hi_memcpy(&pstAwbCtx->stSnsAttrInfo, sizeof(ISP_SNS_ATTR_INFO_S), pstSnsAttrInfo, sizeof(ISP_SNS_ATTR_INFO_S));
...
HI_S32 HI_MPI_AWB_SensorUnRegCallBack(VI_PIPE ViPipe, ALG_LIB_S *pstAwbLib, SENSOR_ID SensorId)
{
    ...
    hi_memset(&pstAwbCtx->stSnsDft, sizeof(AWB_SENSOR_DEFAULT_S), 0, sizeof(AWB_SENSOR_DEFAULT_S));
    hi_memset(&pstAwbCtx->stSnsRegister, sizeof(AWB_SENSOR_REGISTER_S), 0, sizeof(AWB_SENSOR_REGISTER_S));
