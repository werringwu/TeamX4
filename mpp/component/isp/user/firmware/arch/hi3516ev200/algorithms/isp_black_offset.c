...
static HI_VOID GetBuiltInExpanderBlc(const ISP_CMOS_EXPANDER_S *pstSnsExpander, HI_U16 *pu16SensorBlc, HI_U16 *pu16ExpanderBlc)
{
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pu16ExpanderBlc[j] = ((HI_U32)LinearInterpol(pu16SensorBlc[j], au16X[u8IndexLow], au32Y[u8IndexLow], au16X[u8IndexUp], au32Y[u8IndexUp], u8Shift)) >> (1 + u8Shift);
    }
}
...
static HI_VOID BE_BlcDynaRegs_Linear(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        ...
        pstBeBlcCfg->stRgbirBlc.stUsrRegCfg.au16Blc[j] = pstBlackLevel->au16RmDiffBlackLevel[j]; // rgbir
    }
    /* bnr */
...
static HI_VOID BE_BlcDynaRegs_Wdr(VI_PIPE ViPipe, ISP_BE_BLC_CFG_S  *pstBeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    ...
    if (u8WDRModeState == HI_FALSE) { // reg value same as linear mode
        BE_BlcDynaRegs_Linear(ViPipe, pstBeBlcCfg, pstBlackLevel);
    } else if (u8WDRModeState == HI_TRUE) {
        WDROutBlc = ((pstBlackLevel->au16RmDiffBlackLevel[0] >> 4) << 6); /* WDR outblc */
        for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
            for (i = 0; i < ISP_WDR_CHN_MAX; i++) {
                pstBeBlcCfg->st4DgBlc[i].stUsrRegCfg.au16Blc[j]      = pstBlackLevel->au16RmDiffBlackLevel[j] << 2; /* 4DG */
                pstBeBlcCfg->stWdrBlc[i].stUsrRegCfg.au16Blc[j]      = pstBlackLevel->au16RmDiffBlackLevel[j];    /* WDR */
                pstBeBlcCfg->stWdrBlc[i].stUsrRegCfg.u16OutBlc       = WDROutBlc >> 2; /* WDR */
            }
            ...
            pstBeBlcCfg->stRgbirBlc.stUsrRegCfg.au16Blc[j]    = 0;
        }
...
static HI_VOID BE_BlcStaticRegs(HI_U8  u8WdrMode, ISP_BE_BLC_CFG_S  *pstBeBlcCfg)
{
    ...
    /* Rgbir */
    pstBeBlcCfg->stRgbirBlc.stStaticRegCfg.bBlcIn   = HI_TRUE;
    pstBeBlcCfg->stRgbirBlc.stStaticRegCfg.bBlcOut  = HI_TRUE;
    /* lsc */
...
static HI_VOID FE_BlcDynaRegs(ISP_FE_BLC_CFG_S  *pstFeBlcCfg, ISP_BLACKLEVEL_S *pstBlackLevel)
{
    HI_U8 j;
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        pstFeBlcCfg->stFeBlc.stUsrRegCfg.au16Blc[j]   = (pstBlackLevel->au16BlackLevel[j] - pstBlackLevel->au16RmDiffBlackLevel[j]) << 2; /* Fe Blc */
        ...
        pstFeBlcCfg->stRcBlc.stUsrRegCfg.au16Blc[j]   = pstBlackLevel->au16RmDiffBlackLevel[j]  << 2; /* Fe RC */

    }

    pstFeBlcCfg->bReshDyna = HI_TRUE;
    pstFeBlcCfg->bFeBlcCfgChanged = HI_FALSE;
    for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
        if (pstFeBlcCfg->u16FeBlcCfgSave[j] != pstFeBlcCfg->stFeBlc.stUsrRegCfg.au16Blc[j]) {
            pstFeBlcCfg->bFeBlcCfgChanged = HI_TRUE;
        }
        pstFeBlcCfg->u16FeBlcCfgSave[j] = pstFeBlcCfg->stFeBlc.stUsrRegCfg.au16Blc[j];
    }
...
static HI_VOID BlcRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        ...
        pstRegCfg->stAlgRegCfg[i].stWdrRegCfg.stSyncRegCfg.u16Offset0 = pstRegCfg->stAlgRegCfg[i].stBeBlcCfg.stWdrBlc[0].stUsrRegCfg.au16Blc[1];
    }
...
HI_S32 ISP_BlcRun(VI_PIPE ViPipe, const HI_VOID *pStatInfo, HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (hi_ext_system_dpc_static_defect_type_read(ViPipe) == 0) { // hot pixel
        if (pstIspCtx->stLinkage.bDefectPixel) {
            if (pstBlackLevel->bPreDefectPixel == HI_FALSE) {
                for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
    ...
    if (((pstBlackLevel->u8BlackLevelChange == HI_TRUE) && \
         (pstBlackLevel->enOpType == OP_TYPE_AUTO)) || \
        (bWDRStateChange == HI_TRUE)) {
        ISP_SensorUpdateBlc(ViPipe);
        memcpy(&pstBlackLevel->au16BlackLevel[0], &pstSnsBlackLevel->au16BlackLevel[0], ISP_BAYER_CHN_NUM * sizeof(HI_U16));
