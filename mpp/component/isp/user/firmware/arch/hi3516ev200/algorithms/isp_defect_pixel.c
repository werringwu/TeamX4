...
static const HI_U8  g_au8SlopeGrade[ISP_DPC_SLOPE_GRADE]          =   {0, 76, 99, 100, 127};

static const HI_U8  g_au8SoftSlopeGrade[ISP_DPC_SOFT_SLOPE_GRADE]   = {0, 76, 100, 115, 120};
static const HI_U16 g_au16SoftLineThr[ISP_DPC_SOFT_SLOPE_GRADE]     = {0x5454, 0x1818, 0x1212, 0x0a0a, 0x0a0a};
static const HI_U16 g_au16SoftLineMadFac[ISP_DPC_SOFT_SLOPE_GRADE]  = {0x1810, 0x1810, 0x1810, 0x1010, 0x0a0a};
...
typedef struct hiISP_DEFECT_PIXEL_S {
    ...
    HI_U16  u16ProcInfoBlendRatio;
    HI_U16  u16ProcInfoStrength;
...
typedef struct {
    ...
    HI_U16 au16DpccRndThrMtp[2][3];
    HI_U16 au16DpccRgFacMtp[2][3];
...
typedef struct {
    HI_U16 u16DpcLineThr1;
    HI_U16 u16DpcLineMadFac1;
    HI_U16 u16DpcPgFac1;
    HI_U16 u16DpcRndThr1;
    HI_U16 u16DpcRgFac1;
    HI_U16 u16DpcRoLimits1;
    HI_U16 u16DpcRndOffs1;
...
static HI_VOID DpDynaRegsInitialize(VI_PIPE ViPipe, ISP_DPC_DYNA_CFG_S *pstDynaRegCfg)
{
    ISP_CTX_S *pstIspCtx = HI_NULL;
    ...
    pstDynaRegCfg->au16DpccLineStdThr[0]  = HI_ISP_DPC_DEFAULT_LINE_STD_THR_1 * 9;
    pstDynaRegCfg->au16DpccLineStdThr[1]  = HI_ISP_DPC_DEFAULT_LINE_STD_THR_2 * 9;
    pstDynaRegCfg->au16DpccLineStdThr[2]  = HI_ISP_DPC_DEFAULT_LINE_STD_THR_3 * 9;
    pstDynaRegCfg->au16DpccLineStdThr[3]  = HI_ISP_DPC_DEFAULT_LINE_STD_THR_4 * 9;
    pstDynaRegCfg->au16DpccLineStdThr[4]  = HI_ISP_DPC_DEFAULT_LINE_STD_THR_5 * 9;
    ...
    if (IS_LINEAR_MODE(pstIspCtx->u8SnsWDRMode)) {
        pstDynaRegCfg->u8DPCPos = HI_ISP_DPC_DEFAULT_POS_LINEAR;
    } else {
        pstDynaRegCfg->u8DPCPos  = HI_ISP_DPC_DEFAULT_POS_WDR;
    }
}
...
static HI_S32 DpcCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_DPC_S *pstCmosDpc)
{
    ...
    for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++) {
        ...
        if (pstCmosDpc->au16BlendRatio[i] > 0x80) {
            ISP_TRACE(HI_DBG_ERR, "err au16BlendRatio[%d]!\n", i);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
    }
...
static HI_S32 MergingDpCalibLut(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum)
{
    ...
    memset(pu32BpTable, 0, pstDp->u16BptCalibNum * sizeof(HI_U32));
...
static HI_VOID Dpc_Calib_Success(VI_PIPE ViPipe, HI_U8 u8BlkNum, ISP_DEFECT_PIXEL_S *pstDp, ISP_STAT_S *pStatInfo)
{
    ...
    if (IS_ONLINE_MODE(pstIspCtx->stBlockAttr.enIspRunningMode)) {
        ...
        for (j = 0; j < pstDp->u16BptCalibNum; j++) {
            hi_ext_system_dpc_calib_bpt_write(ViPipe, j, pStatInfo->stDpStat.au32DefectPixelLut[0][j]);
        }
    } else {
...
static HI_VOID Dpc_Hot_Calib(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum, ISP_REG_CFG_S *pstRegCfg, ISP_STAT_S *pStatInfo)
{
    if (pstDp->u8FrameCnt < 9) {
        ...
        if (pstDp->u8FrameCnt == 3) {
            pstDp->u32DpccBadThresh = ((HI_U32)pstDp->u8StaticDPThresh << 24) + (((50 + 0x80 * pstDp->u8HotDevThresh) / 100) << 16) + 0x00000080;
...
static HI_VOID Dpc_Dark_Calib(VI_PIPE ViPipe, ISP_DEFECT_PIXEL_S  *pstDp, HI_U8 u8BlkNum, ISP_REG_CFG_S *pstRegCfg, ISP_STAT_S *pStatInfo)
{
    if (pstDp->u8FrameCnt < 9) {
        ...
        if (pstDp->u8FrameCnt == 3) {
            pstDp->u32DpccBadThresh = 0xFF800000 + ((HI_U32)pstDp->u8StaticDPThresh << 8) + ((0x80 * pstDp->u8DeadDevThresh) / 100);
...
static HI_VOID set_dpcc_parameters_inter(ISP_DPC_DYNA_CFG_S *pstIspDpccHwCfg, HI_U8 u8DpccStat)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        pstIspDpccHwCfg->au32DpccRndThrMtp[j]  = ((HI_U16)(stDpcDerParam.au16DpccRndThrMtp[0][j] & 0x3FF) << 10) + (stDpcDerParam.au16DpccRndThrMtp[1][j] & 0x3FF);
        pstIspDpccHwCfg->au32DpccRgFacMtp[j]   = ((HI_U16)(stDpcDerParam.au16DpccRgFacMtp[0][j]  & 0x3FF) << 10) + (stDpcDerParam.au16DpccRgFacMtp[1][j]  & 0x3FF);
    }

    pstIspDpccHwCfg->u16DpccRoLimits    = ((HI_U16)(stDpcDerParam.au8DpccRo[0][2] & 0x3) << 10) + ((HI_U16)(stDpcDerParam.au8DpccRo[1][2] & 0x3) << 8) + \
    ...
    pstIspDpccHwCfg->u16DpccRndOffs     = ((HI_U16)(stDpcDerParam.au8DpccRndOffs[0][2] & 0x3) << 10) + ((HI_U16)(stDpcDerParam.au8DpccRndOffs[1][2] & 0x3) << 8) + \
...
static HI_S32 ISP_Dynamic_set(HI_U32 u32Iso, ISP_DPC_DYNA_CFG_S *pstDpcHwCfg, ISP_DEFECT_PIXEL_S *pstDpcFwCfg)
{
    ...
    if (u8DpccStat == 0) {
        ...
        pstDpcHwCfg->au32DpccRndThrMtp[0]      = ((HI_U16)(stDpcDerParam.au16DpccRndThrMtp[0][0] & 0x3FF) << 10) + (stDpcDerParam.au16DpccRndThrMtp[1][0] & 0x3FF);
        pstDpcHwCfg->au32DpccRgFacMtp[0]       = ((HI_U16)(stDpcDerParam.au16DpccRgFacMtp[0][0]  & 0x3FF) << 10) + (stDpcDerParam.au16DpccRgFacMtp[1][0]  & 0x3FF);
    ...
    pstDpcHwCfg->u32DpccAlpha  = ((HI_U32)u8Alpha0RB << 24) + (u8Alpha0G << 16) + (u8Alpha1RB << 8) + u8Alpha1G;
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstIspCtx->stLinkage.bStatReady == HI_FALSE) {
        return HI_SUCCESS;
    }
...
HI_S32 DpcProcWrite(VI_PIPE ViPipe, ISP_CTRL_PROC_WRITE_S *pstProc)
{
    ...
    ISP_PROC_PRINTF(&stProcTmp, pstProc->u32WriteLen,
                    "%12u" "%12u"  "%12u\n",
                    pstDp->bEnable, pstDp->u16ProcInfoStrength, pstDp->u16ProcInfoBlendRatio);
