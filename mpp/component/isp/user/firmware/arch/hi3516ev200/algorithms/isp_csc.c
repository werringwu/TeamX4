...
static HI_VOID CscDynaRegsConfig(ISP_CSC_ATTR *pstCscAttr, ISP_CSC_DYNA_CFG_S *pstDynaRegCfg)
{
    ...
    pstDynaRegCfg->s16CscInDC0  = SignedLeftShift(pstCscAttr->as16CSCIdc[0], 2);
    pstDynaRegCfg->s16CscInDC1  = SignedLeftShift(pstCscAttr->as16CSCIdc[1], 2);
    pstDynaRegCfg->s16CscInDC2  = SignedLeftShift(pstCscAttr->as16CSCIdc[2], 2);
    pstDynaRegCfg->s16CscOutDC0 = SignedLeftShift(pstCscAttr->as16CSCOdc[0], 2);
    pstDynaRegCfg->s16CscOutDC1 = SignedLeftShift(pstCscAttr->as16CSCOdc[1], 2);
    pstDynaRegCfg->s16CscOutDC2 = SignedLeftShift(pstCscAttr->as16CSCOdc[2], 2);
...
HI_S32 CscCalMatrix(VI_PIPE ViPipe, ISP_CSC_S *pstCsc, ISP_CSC_ATTR *pstCscCoef)
{
    ...
    if ((pstCsc->bCtmodeEn) && (pstCsc->enColorGamut <= COLOR_GAMUT_USER)) {
        s16BlackIn  = -128;
        s16BlackOut = (pstCsc->bLimitedRangeEn) ? 110 : 128;    /* 110 = (128 * 219) / 256, Full range to limited range conversion */
    } else {
        s16BlackIn  = 0;
        s16BlackOut = 0;
    }
                  ...
                  HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        // Coefficiets config
        CscDynaRegsConfig(pstCscAttr, &pstRegCfg->stAlgRegCfg[i].stCscCfg.stDynaRegCfg);
