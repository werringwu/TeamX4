...
static HI_VOID AeUpdateLinkage(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_LINKAGE_S *pstLinkage)
{
    ...
    if (pstIspCtx->stLinkage.bRunOnce == HI_TRUE) {
    ...
    if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode)) {
        ...
        u32ExpRatioTmp = ((HI_U64)(pstAeResult->u32IntTime[1]) * (pstAeResult->u32Iso << 2) + (((HI_U64)pstAeResult->u32IntTime[0] * pstAeResult->u32IsoSF) >> 4 ) / 2) / DIV_0_TO_1(((HI_U64)pstAeResult->u32IntTime[0] * pstAeResult->u32IsoSF) >> 4);
...
HI_S32 ISP_AeInit(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pstIspCtx->stAeLibInfo.astLibs[i].bUsed) {
            ...
            if (pstLib->stAeRegsiter.stAeExpFunc.pfn_ae_init != HI_NULL) {
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstLib->stAeRegsiter.stAeExpFunc.pfn_ae_ctrl != HI_NULL) {
    ...
    if (pstLib->stAeRegsiter.stAeExpFunc.pfn_ae_run != HI_NULL) {
...
HI_S32 ISP_AeExit(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pstIspCtx->stAeLibInfo.astLibs[i].bUsed) {
            pstLib = &pstIspCtx->stAeLibInfo.astLibs[i];
            if (pstLib->stAeRegsiter.stAeExpFunc.pfn_ae_exit != HI_NULL) {
