...
static const  HI_S8 g_as8PreGammaLutLogDelta[257] = {
...
*/
static HI_U32 PreGammaNonStandardLog2(HI_U32 x, HI_U8 ShpLog)
{
    HI_S8  k;
    HI_U8  StepShift = 7;
    HI_U32 IntPart, FracPart;
    HI_U32 NumerOffset, DivDenomLog;
    HI_S32 Offset, LUTf;
    HI_U32 y;
    ...
    x = (HI_U32)MIN2((1LL << 32) - 1, (HI_U64)x + (1LL << ShpLog));
    ...
    LUTf = (HI_S32)g_au16PreGammaLutLog[FracPart >> StepShift] + \
           (HI_S32)SignedRightShift(((HI_S64)g_as8PreGammaLutLogDelta[FracPart >> StepShift] * Offset + (1 << (StepShift - 1))), StepShift);
    y = (HI_U32)((IntPart << PREGAMMA_FRAC_BIT_DEP) + (FracPart << 1) + LUTf);
    y = (HI_U32)((((HI_U64)y - NumerOffset) * DivDenomLog) >> (PREGAMMA_DENOM_SHIFT_LOG + PREGAMMA_FRAC_BIT_DEP - PREGAMMA_LOG_BIT_DEP)); // y > NumerOffset for sure

    return y;
}

static const HI_U32 g_au32PreGammaLinearDefault[PREGAMMA_NODE_NUM] = {
...
static const HI_U8 g_au8PregammaSegIdxBase[HI_ISP_PREGAMMA_SEG_NUM] = {0, 4, 12, 24, 38, 53, 77, 105, 135, 168, 200, 224};
static const HI_U16 g_au16PregammaSegMaxVal[HI_ISP_PREGAMMA_SEG_NUM] = {1, 4, 12, 28, 60, 192, 448, 960, 2112, 4096, 6144, 8192};
...
typedef struct hiISP_PREGAMMA_S {
    ...
    HI_U8   u8ShpLog;
    HI_U8   u8ShpLogPrev;
    HI_U16  au16PreGammaHw[PREGAMMA_NODE_NUM];
} ISP_PREGAMMA_S;
...
static HI_S32 PreGammaCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_PREGAMMA_S *pstPreGamma)
{
    ...
    for (i = 0; i < PREGAMMA_NODE_NUM; i++) {
        if (pstPreGamma->au32PreGamma[i] > HI_ISP_PREGAMMA_LUT_MAX) {
            ISP_TRACE(HI_DBG_ERR, "err au32PreGamma[%d]!\n", i);
...
static HI_S32 PreGammaRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    if (pstSnsDft->unKey.bit1Drc) {
        ...
        pstPreGammaCtx->u8ShpLog = HI_ISP_PREGAMMA_SHP_LOG_DEFAULT;
    }
    ...
    if (pstSnsDft->unKey.bit1PreGamma) {
        ...
        for (i = 0; i < PREGAMMA_NODE_NUM; i++) {
            pstPreGammaCtx->au16PreGammaHw[i] = (HI_U16)MIN2(65535, PreGammaNonStandardLog2(pstSnsDft->pstPreGamma->au32PreGamma[i], pstPreGammaCtx->u8ShpLog));
        }
#ifndef VREG_OLD
        for (i = 0; i < PREGAMMA_NODE_NUM; i++) {
            hi_ext_system_pregamma_lut_write(ViPipe, i, pstSnsDft->pstPreGamma->au32PreGamma[i]);
        }
#else
        u32Addr   = VREG_PREGAMMA_BASE(ViPipe) + 0x4;
        pVirtAddr = VReg_GetVirtAddr(u32Addr);
        memcpy((HI_U32 *)pVirtAddr, pstSnsDft->pstPreGamma->au32PreGamma, PREGAMMA_NODE_NUM * sizeof(HI_U32));
#endif
        pstPreGammaCtx->bEnable    = pstSnsDft->pstPreGamma->bEnable;
    } else {
        for (i = 0; i < PREGAMMA_NODE_NUM; i++) {
            pstPreGammaCtx->au16PreGammaHw[i] = (HI_U16)MIN2(65535, PreGammaNonStandardLog2(g_au32PreGammaLinearDefault[i], pstPreGammaCtx->u8ShpLog));
        }


#ifndef VREG_OLD
        for (i = 0; i < PREGAMMA_NODE_NUM; i++) {
            hi_ext_system_pregamma_lut_write(ViPipe, i, g_au32PreGammaLinearDefault[i]);
        }
#else
        u32Addr   = VREG_PREGAMMA_BASE(ViPipe) + 0x4;
        pVirtAddr = VReg_GetVirtAddr(u32Addr);
        memcpy((HI_U32 *)pVirtAddr, g_au32PreGammaLinearDefault, PREGAMMA_NODE_NUM * sizeof(HI_U32));
#endif
    ...
    for (i = 0; i < u8BlockNum; i++) {
        ...
        memcpy(pstPreGammaCfg->stStaticRegCfg.au8SegIdxBase, g_au8PregammaSegIdxBase, HI_ISP_PREGAMMA_SEG_NUM * sizeof(HI_U8));
        memcpy(pstPreGammaCfg->stStaticRegCfg.au16SegMaxVal,  g_au16PregammaSegMaxVal,  HI_ISP_PREGAMMA_SEG_NUM * sizeof(HI_U16));
        ...
        memcpy(pstPreGammaCfg->stDynaRegCfg.au16PreGammaLUT, pstPreGammaCtx->au16PreGammaHw, PREGAMMA_NODE_NUM * sizeof(HI_U16));
    }
...
static HI_S32 PreGammaReadExtRegs(VI_PIPE ViPipe)
{
    ...
    pstPreGammaCtx->u8ShpLogPrev = pstPreGammaCtx->u8ShpLog;
    pstPreGammaCtx->u8ShpLog = hi_ext_system_drc_shp_log_read(ViPipe);
    ...
    for (i = 0; i < pstReg->u8CfgNum; i++) {
        ...
        if (pstPreGammaCtx->bLutUpdate || pstPreGammaCtx->u8ShpLogPrev != pstPreGammaCtx->u8ShpLog) {
            for (j = 0; j < PREGAMMA_NODE_NUM; j++) {
                u32PreGammaVal = MIN2(hi_ext_system_pregamma_lut_read(ViPipe, j), HI_ISP_PREGAMMA_LUT_MAX);
                pstDynaRegCfg->au16PreGammaLUT[j] = (HI_U16)MIN2(65535, PreGammaNonStandardLog2(u32PreGammaVal, pstPreGammaCtx->u8ShpLog));
            }
