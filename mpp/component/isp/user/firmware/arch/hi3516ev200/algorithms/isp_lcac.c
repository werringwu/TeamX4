...
typedef struct hiISP_CAC_S {
    ...
    HI_S32  ProcInfoDePurpleCtrCb; // [0, 8]
    HI_S32  ProcInfoDePurpleCtrCr; // [0, 8]
...
static HI_VOID local_cac_usr_fw(ISP_LOCAL_CAC_USR_CFG_S  *pstLCacUsrRegCfg, ISP_LCAC_S *pstLocalCAC)
{
    ...
    if (pstLCacUsrRegCfg->u8CbCntLowThr == pstLCacUsrRegCfg->u8CbCntHighThr) {
        pstLCacUsrRegCfg->u16CbCntRatio = 0;
    } else {
        pstLCacUsrRegCfg->u16CbCntRatio = 64 * 16 / DIV_0_TO_1(MAX2((pstLCacUsrRegCfg->u8CbCntHighThr - pstLCacUsrRegCfg->u8CbCntLowThr), 0));
    }
...
HI_S32 LcacProcWrite(VI_PIPE ViPipe, ISP_CTRL_PROC_WRITE_S *pstProc)
{
    ...
    ISP_PROC_PRINTF(&stProcTmp, pstProc->u32WriteLen,
                    "%12u" "%12u" "%12u\n",
                    pstLocalCAC->bLocalCacEn, pstLocalCAC->ProcInfoDePurpleCtrCr, pstLocalCAC->ProcInfoDePurpleCtrCb);
