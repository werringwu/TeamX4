...
HI_S32 AwbCfgInitReg(VI_PIPE ViPipe, ISP_AWB_RESULT_S *pstAwbResult, ISP_REG_CFG_S *pstRegCfg)
{
    HI_S32 i, k;
    HI_U32 au32WbGain[4];
    HI_U8 u8BlkNum;
...
HI_S32 ISP_AwbInit(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pstIspCtx->stAwbLibInfo.astLibs[i].bUsed) {
            ...
            if (pstLib->stAwbRegsiter.stAwbExpFunc.pfn_awb_init != HI_NULL) {
                  ...
                  HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstIspCtx->stLinkage.bSnapState == HI_TRUE) {
        if (bLoadCCM == HI_TRUE) {
            ...
            if (pstLib->stAwbRegsiter.stAwbExpFunc.pfn_awb_ctrl != HI_NULL) {
    ...
    if (pstLib->stAwbRegsiter.stAwbExpFunc.pfn_awb_run != HI_NULL) {
...
HI_S32 ISP_AwbExit(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < MAX_REGISTER_ALG_LIB_NUM; i++) {
        if (pstIspCtx->stAwbLibInfo.astLibs[i].bUsed) {
            ...
            if (pstLib->stAwbRegsiter.stAwbExpFunc.pfn_awb_exit != HI_NULL) {
