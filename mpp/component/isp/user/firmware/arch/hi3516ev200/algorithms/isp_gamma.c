...
typedef struct hiISP_GAMMA_S {
    ...
    HI_U8   au8Step[GAMMA_OUTSEG_NUMBER];
    HI_U16  au16InSeg[GAMMA_OUTSEG_NUMBER];
    HI_U16  au16Pos[GAMMA_OUTSEG_NUMBER];

    HI_U16  *pu16GAMMA;
    HI_U16  *pu16ExtGAMMA;
} ISP_GAMMA_S;

ISP_GAMMA_S *g_astGammaCtx[ISP_MAX_PIPE_NUM] = {HI_NULL};

#define GAMMA_GET_CTX(dev, pstCtx)   (pstCtx = g_astGammaCtx[dev])
#define GAMMA_SET_CTX(dev, pstCtx)   (g_astGammaCtx[dev] = pstCtx)
#define GAMMA_RESET_CTX(dev)         (g_astGammaCtx[dev] = HI_NULL)

static const HI_U8  g_au8NonUniStepLut[GAMMA_OUTSEG_NUMBER]   = {2, 2, 3, 4, 4, 4, 4, 5};
static const HI_U16 g_au16NonUniInSegLut[GAMMA_OUTSEG_NUMBER] = {16, 32, 48, 64, 96, 160, 224, 256};

// Dafault Gamma curve
static const HI_U16 g_au16LinModeGamma[GAMMA_EXT_NODE_NUMBER] = {
...
static const HI_U16 g_au16WDRModeGamma[GAMMA_EXT_NODE_NUMBER] = {
...
HI_S32 GammaCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pstGamma == HI_NULL) {
        ...
        if (pstGamma == HI_NULL) {
            ISP_TRACE(HI_DBG_ERR, "Isp[%d] GammaCtx malloc memory failed!\n", ViPipe);
...
HI_VOID GammaCtxExit(VI_PIPE ViPipe)
{
    ISP_GAMMA_S *pstGamma = HI_NULL;
...
HI_VOID GammaGetPos(ISP_GAMMA_S *pstGamma, HI_U16 *pau16ActualLut, HI_U16 *pau16InputLut)
{
    // This function is used to generate Pos value from InSeg and Step we entered
    HI_U16 i, j;
    HI_U16 InSegNum[GAMMA_OUTSEG_NUMBER]; // Number of Inner Segments in each Outer Segment, thus the first InSegNum should be the first number of au16InSeg;
    HI_U16 Point[GAMMA_OUTSEG_NUMBER]; // Position of each start of the Outer Segment, thus the first Point should always be 0;
    HI_U16 Step[GAMMA_OUTSEG_NUMBER];  // Acutual step value of each Outer Segment
    HI_U16 LutPos[GAMMA_NODE_NUMBER];
    ...
    //
    /*****************************************************************************/
    HI_U16 SegStart, SegEnd, SegPosStart;
    ...
    for (i = 1; i < GAMMA_OUTSEG_NUMBER; i++) {
        InSegNum[i] = pstGamma->au16InSeg[i] - pstGamma->au16InSeg[i - 1];
        pstGamma->au8Step[i - 1] = CLIP_MAX(pstGamma->au8Step[i - 1], 7);
        Point[i] = Point[i - 1] + InSegNum[i - 1] * (1 << pstGamma->au8Step[i - 1]);
        Step[i] = 1 << pstGamma->au8Step[i];
        pstGamma->au16Pos[i - 1] = Point[i];
    }

    pstGamma->au16Pos[GAMMA_OUTSEG_NUMBER - 1] = Point[GAMMA_OUTSEG_NUMBER - 1] + InSegNum[GAMMA_OUTSEG_NUMBER - 1] * (1 << pstGamma->au8Step[GAMMA_OUTSEG_NUMBER - 1]);
    ...
    pau16ActualLut[GAMMA_NODE_NUMBER - 1] = pau16InputLut[GAMMA_EXT_NODE_NUMBER - 1];

}

static HI_VOID GammaUsrRegsInit(ISP_GAMMA_USR_CFG_S *pstUsrRegCfg, HI_U16 *pu16Gamma, ISP_GAMMA_S *pstGamma)
{
    ...
    memcpy(pstUsrRegCfg->au16GammaInSeg, pstGamma->au16InSeg, GAMMA_OUTSEG_NUMBER * sizeof(HI_U16));
    memcpy(pstUsrRegCfg->au16GammaPos,   pstGamma->au16Pos,   GAMMA_OUTSEG_NUMBER * sizeof(HI_U16));
    memcpy(pstUsrRegCfg->au8GammaStep,   pstGamma->au8Step,   GAMMA_OUTSEG_NUMBER * sizeof(HI_U8));
    memcpy(pstUsrRegCfg->au16GammaLUT,   pu16Gamma,           GAMMA_NODE_NUMBER   * sizeof(HI_U16));
}
...
static HI_VOID GammaRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        GammaUsrRegsInit(&pstRegCfg->stAlgRegCfg[i].stGammaCfg.stUsrRegCfg, pstGammaCtx->pu16GAMMA, pstGammaCtx);
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.bGammaEn   = HI_TRUE;
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.bLut2SttEn = HI_TRUE;
    }
...
static HI_S32 GammaReadExtRegs(VI_PIPE ViPipe)
{
    ...
    if (pstGammaCtx->bLutUpdate) {
        for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++) {
            pstGammaCtx->pu16ExtGAMMA[i] = hi_ext_system_gamma_lut_read(ViPipe, i);
        }
    }
...
static HI_S32 GammaCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_GAMMA_S *pstGamma)
{
    HI_U16 i;

    for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++) {
        if (pstGamma->au16Gamma[i] > 0xFFF) {
            ISP_TRACE(HI_DBG_ERR, "err au16Gamma[%d]!\n", i);
...
static HI_S32 GammaInitialize(VI_PIPE ViPipe)
{
    HI_U8  u8WDRMode;
    HI_U16 i;
    HI_S32 s32Ret;
    ...
    if (pstGammaCtx->pu16GAMMA == HI_NULL) {
        pstGammaCtx->pu16GAMMA = (HI_U16 *)ISP_MALLOC(GAMMA_NODE_NUMBER * sizeof(HI_U16));

        if (pstGammaCtx->pu16GAMMA == HI_NULL) {
            ISP_TRACE(HI_DBG_ERR, "malloc pstGammaCtx->pau16GAMMA failed!\n");
            return HI_FAILURE;
        }
    }

    if (pstGammaCtx->pu16ExtGAMMA == HI_NULL) {
        pstGammaCtx->pu16ExtGAMMA = (HI_U16 *)ISP_MALLOC(GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));

        if (pstGammaCtx->pu16ExtGAMMA == HI_NULL) {
            ISP_TRACE(HI_DBG_ERR, "malloc pstGammaCtx->pau16ExtGAMMA failed!\n");
    ...
    if (pstSnsDft->unKey.bit1Gamma) {
        ...
        memcpy(pstGammaCtx->pu16ExtGAMMA, pstSnsDft->pstGamma->au16Gamma, GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));
        ...
        if (IS_LINEAR_MODE(u8WDRMode)) {
            memcpy(pstGammaCtx->pu16ExtGAMMA, g_au16LinModeGamma, GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));
        } else {
            memcpy(pstGammaCtx->pu16ExtGAMMA, g_au16WDRModeGamma, GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));
        }
...
#ifndef VREG_OLD
    for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++) {
        hi_ext_system_gamma_lut_write(ViPipe, i, pstGammaCtx->pu16ExtGAMMA[i]);
    }
#else
    HI_U32 u32Addr;
    ...
    memcpy((HI_U16 *)pVirtAddr, pstGammaCtx->pu16ExtGAMMA, GAMMA_EXT_NODE_NUMBER * sizeof(HI_U16));
#endif
    // Generate Pos by using InSeg and Step
    GammaGetPos(pstGammaCtx, pstGammaCtx->pu16GAMMA, pstGammaCtx->pu16ExtGAMMA);
...
static HI_VOID ISP_GammaWdrModeSet(VI_PIPE ViPipe, HI_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        ...
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.stUsrRegCfg.bSwitchMode    = HI_TRUE;
    }
}
                    ...
                    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (HI_NULL == pstGammaCtx->pu16GAMMA || HI_NULL == pstGammaCtx->pu16ExtGAMMA) {
        ISP_TRACE(HI_DBG_ERR, "Gamma lutCtx is empty!\n");
        return HI_FAILURE;
    }
    ...
    if (pstGammaCtx->bLutUpdate) {
        ...
        for (i = 0; i < pstReg->u8CfgNum; i++) {
            ...
            memcpy(pstUsrRegCfg->au16GammaLUT, pstGammaCtx->pu16GAMMA, GAMMA_NODE_NUMBER * sizeof(HI_U16));
