...
ISP_FOCUS_STATISTICS_CFG_S  g_stFocusCfg[ISP_MAX_PIPE_NUM] = {
    ...
    {
        {1, 17, 15, 1920, 1080, 1, 0, {0, 0, 0, 1920, 1080}, 0, {0x2, 0x1, 0}, {1, 0x9bff}, 0xf0},
...
HI_S32 ISP_AfExtRegsDefault(VI_PIPE ViPipe)
{
    ...
    u8AfEnable = (g_stFocusCfg[ViPipe].stConfig.bEnable == HI_TRUE) ? 0x3 : 0x0;
    ...
    g_stFocusCfg[ViPipe].stConfig.u16Hsize = pstIspCtx->stBlockAttr.stFrameRect.u32Width;
    g_stFocusCfg[ViPipe].stConfig.u16Vsize = pstIspCtx->stBlockAttr.stFrameRect.u32Height;
...
HI_S32 ISP_SetAfBeDefault(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < u8BlockNum; i++) {
        ...
        pstAFdefaultCfg->u16FirSlope1Coring = g_stFocusCfg[ViPipe].stVParam_FIR1.stCoring.u16Slp;
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (hi_ext_af_set_flag_read(ViPipe) == HI_EXT_AF_SET_FLAG_ENABLE) {
        hi_ext_af_set_flag_write(ViPipe, HI_EXT_AF_SET_FLAG_DISABLE);
