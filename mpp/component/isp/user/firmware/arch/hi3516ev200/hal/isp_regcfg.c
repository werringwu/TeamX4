...
HI_S32 ISP_ModParamGet(ISP_MOD_PARAM_S *pstModParam)
{
    ...
    if (ioctl(g_as32IspFd[0], ISP_GET_MOD_PARAM, pstModParam) != HI_SUCCESS) {
...
HI_S32 ISP_BeLutBufAddrExit(VI_PIPE ViPipe)
{
    ...
    if (pstBeLutBuf->astLutSttBuf[0].pVirAddr != HI_NULL) {
...
static HI_S32 ISP_UpdateLdciReadSttBufCtx(VI_PIPE ViPipe, HI_U64 u64PhyAddr)
{
    ...
    HI_VOID   *pVirtAddr = HI_NULL;
    ISP_CTX_S *pstIspCtx = HI_NULL;
    ISP_LDCI_READ_STT_BUF_S *pstLdciReadSttBuf = HI_NULL;

    ISP_GET_CTX(ViPipe, pstIspCtx);

    if (pstIspCtx->bLdciTprFltEn == HI_FALSE) {
        return HI_SUCCESS;
    }
    ...
    pVirtAddr = HI_MPI_SYS_Mmap(u64PhyAddr, u64Size * pstLdciReadSttBuf->u8BufNum);

    if (pVirtAddr == HI_NULL) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d]:map ldci read stt buffer failed\n", ViPipe);
        return HI_FAILURE;
    }

    for (i = 0; i < pstLdciReadSttBuf->u8BufNum; i++) {
        pstLdciReadSttBuf->astReadBuf[i].u64PhyAddr = u64PhyAddr + i * u64Size;
        pstLdciReadSttBuf->astReadBuf[i].pVirAddr   = (HI_VOID *)((HI_U8 *)pVirtAddr + i * u64Size);
...
HI_S32 ISP_LdciReadSttBufAddInit(VI_PIPE ViPipe)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d]:Get ldci read stt buffer address failed\n", ViPipe);
        return s32Ret;
    }
...
HI_S32 ISP_LdciReadSttBufAddExit(VI_PIPE ViPipe)
{
    ...
    if (pstLdciReadSttBuf->astReadBuf[0].pVirAddr != HI_NULL) {
        HI_MPI_SYS_Munmap(pstLdciReadSttBuf->astReadBuf[0].pVirAddr,
                          sizeof(ISP_LDCI_STAT_S) * pstLdciReadSttBuf->u8BufNum);

        for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
            pstLdciReadSttBuf->astReadBuf[i].pVirAddr = HI_NULL;
        }
    }
...
HI_S32 ISP_LdciBufInit(VI_PIPE ViPipe)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] get ctrlparam failed\n", ViPipe);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] ldci buffer init failed\n", ViPipe);
        return HI_FAILURE;
    }

    if (pstIspCtx->bLdciTprFltEn == HI_TRUE) {
        s32Ret = ISP_LdciReadSttBufAddInit(ViPipe);

        if (s32Ret != HI_SUCCESS) {
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] ldci read stt buffer address init failed\n", ViPipe);
...
HI_S32 ISP_LdciBufExit(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->bLdciTprFltEn == HI_TRUE) {
        ...
        if (s32Ret != HI_SUCCESS) {
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] exit readstt bufaddr failed\n", ViPipe);
            return HI_FAILURE;
        }
    }

    if (ioctl(g_as32IspFd[ViPipe], ISP_LDCI_BUF_EXIT) != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] exit ldci bufs failed\n", ViPipe);
        return HI_FAILURE;
    }
...
HI_S32 ISP_CfgBeBufInit(VI_PIPE ViPipe)
{
    ...
    pstBeBuf->pBeVirtAddr = HI_MPI_SYS_MmapCache(pstBeBuf->u64BePhyAddr,
                                                 sizeof(ISP_BE_WO_REG_CFG_S) * MAX_ISP_BE_BUF_NUM);

    if (pstBeBuf->pBeVirtAddr == HI_NULL) {
    ...
    switch (enIspRuningMode) {
        case ISP_MODE_RUNNING_STRIPING:
            for (i = 0; i < ISP_STRIPING_MAX_NUM; i++) {
                ...
                pstIspCtx->pViProcVirtAddr[i] = (HI_VOID *)((HI_U8 *)pstIspCtx->pIspBeVirtAddr[i] +
...
HI_S32 ISP_CfgBeBufMmap(VI_PIPE ViPipe)
{
    ...
    if (pstBeBuf->stBeWoCfgBuf.pVirAddr == HI_NULL) {
        return HI_FAILURE;
    }
    ...
    switch (enIspRuningMode) {
        ...
        case ISP_MODE_RUNNING_OFFLINE:
            for (k = 0; k < ISP_STRIPING_MAX_NUM; k++) {
                if (k == 0) {
                    ...
                    pstIspCtx->pViProcVirtAddr[k] = (HI_VOID *)((HI_U8 *)pstBeBuf->stBeWoCfgBuf.pVirAddr +
                                                                VIPROC_OFFLINE_OFFSET);
                } else {
                    pstIspCtx->pIspBeVirtAddr[k]  = HI_NULL;
                    pstIspCtx->pViProcVirtAddr[k] = HI_NULL;
        ...
        case ISP_MODE_RUNNING_STRIPING:
            for (k = 0; k < ISP_STRIPING_MAX_NUM; k++) {
                ...
                pstIspCtx->pViProcVirtAddr[k] = (HI_VOID *)((HI_U8 *)pstIspCtx->pIspBeVirtAddr[k] +
                                                            VIPROC_OFFLINE_OFFSET);
            }
            break;
...
HI_VOID *ISP_VRegCfgBufAddr(VI_PIPE ViPipe, BLK_DEV BlkDev)
{
    ...
    pstBeBuf->stBeWoCfgBuf.pVirAddr = HI_MPI_SYS_MmapCache(pstBeBuf->stBeWoCfgBuf.u64PhyAddr,
...
HI_VOID *ISP_GetLdciReadSttVirAddr(VI_PIPE ViPipe, HI_U8 u8BufIdx)
{
    ...
    if (pstLdciReadSttBuf->astReadBuf[u8BufIdx].pVirAddr != HI_NULL) {
...
HI_VOID *ISP_GetBeLut2SttVirAddr(VI_PIPE ViPipe, BLK_DEV BlkDev, HI_U8 U8BufId)
{
    ...
    if (pstBeLutBuf->astLutSttBuf[BlkDev].pVirAddr != HI_NULL) {
...
static HI_S32 ISP_AfRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (bUsrResh) {
        ...
        isp_af_shift_u32_write(pstBeReg, pstAfRegBeCfg->u16Iir0ShiftGroup0, pstAfRegBeCfg->u16Iir1ShiftGroup0, pstAfRegBeCfg->u16Iir2ShiftGroup0,
                               pstAfRegBeCfg->u16Iir3ShiftGroup0, pstAfRegBeCfg->u16Iir0ShiftGroup1, pstAfRegBeCfg->u16Iir1ShiftGroup1,
                               pstAfRegBeCfg->u16Iir2ShiftGroup1, pstAfRegBeCfg->u16Iir3ShiftGroup1);

        isp_af_firslope_coring_u32_write(pstBeReg, pstAfRegBeCfg->u16FirSlope0Coring, pstAfRegBeCfg->u16FirSlope1Coring);
        ...
        isp_af_cfg_u32_write(pstBeReg, pstAfRegBeCfg->bIir0Enable0, pstAfRegBeCfg->bIir0Enable1, pstAfRegBeCfg->bIir0Enable2, pstAfRegBeCfg->bIir1Enable0,
                             pstAfRegBeCfg->bIir1Enable1, pstAfRegBeCfg->bIir1Enable2, pstAfRegBeCfg->enPeakMode, pstAfRegBeCfg->enSquMode,
                             pstAfRegBeCfg->bOffsetEnable, pstAfRegBeCfg->bCropEnable, pstAfRegBeCfg->bLpfEnable, pstAfRegBeCfg->bMeanEnable,
                             pstAfRegBeCfg->bRawMode, pstAfRegBeCfg->u8BayerMode, pstAfRegBeCfg->bIir0DsEnable, pstAfRegBeCfg->bIir1DsEnable,
                             pstAfRegBeCfg->bFir0LpfEnable, pstAfRegBeCfg->bFir1LpfEnable, pstAfRegBeCfg->bIir0LdgEnable,
                             pstAfRegBeCfg->bIir1LdgEnable, pstAfRegBeCfg->bFir0LdgEnable, pstAfRegBeCfg->bFir1LdgEnable, pstAfRegBeCfg->u8Gamma);
...
static HI_S32 ISP_FeAwbRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo)
{
    ...
    if (pstIspCtx->stWdrAttr.bMastPipe) {
        for (i = 0; i < pstIspCtx->stWdrAttr.stDevBindPipe.u32Num; i++) {
            ...
            if (pstIspCtx->stLinkage.bSnapState == HI_TRUE) {
...
static HI_S32 ISP_AwbRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstIspCtx->stLinkage.bSnapState == HI_TRUE) {
        if (pstIspCtx->stLinkage.bLoadCCM == HI_TRUE) {
...
static HI_S32 ISP_SharpenDynaRegConfig(S_ISPBE_REGS_TYPE *pstBeReg, ISP_SHARPEN_MPI_DYNA_REG_CFG_S *pstMpiDynaRegCfg, ISP_SHARPEN_DEFAULT_DYNA_REG_CFG_S *pstDefDynaRegCfg)
{
    ...
    isp_sharpen_lumawgt3_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[15], pstMpiDynaRegCfg->au8LumaWgt[16], pstMpiDynaRegCfg->au8LumaWgt[17],
                                   pstMpiDynaRegCfg->au8LumaWgt[18], pstMpiDynaRegCfg->au8LumaWgt[19]);

    isp_sharpen_lumawgt4_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[20], pstMpiDynaRegCfg->au8LumaWgt[21], pstMpiDynaRegCfg->au8LumaWgt[22],
                                   pstMpiDynaRegCfg->au8LumaWgt[23], pstMpiDynaRegCfg->au8LumaWgt[24]);

    isp_sharpen_lumawgt5_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[25], pstMpiDynaRegCfg->au8LumaWgt[26], pstMpiDynaRegCfg->au8LumaWgt[27],
                                   pstMpiDynaRegCfg->au8LumaWgt[28], pstMpiDynaRegCfg->au8LumaWgt[29]);

    isp_sharpen_lumawgt2_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[10], pstMpiDynaRegCfg->au8LumaWgt[11], pstMpiDynaRegCfg->au8LumaWgt[12],
                                   pstMpiDynaRegCfg->au8LumaWgt[13], pstMpiDynaRegCfg->au8LumaWgt[14]);

    isp_sharpen_lumawgt0_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[0], pstMpiDynaRegCfg->au8LumaWgt[1], pstMpiDynaRegCfg->au8LumaWgt[2],
                                   pstMpiDynaRegCfg->au8LumaWgt[3], pstMpiDynaRegCfg->au8LumaWgt[4]);

    isp_sharpen_lumawgt1_u32_write(pstBeReg, pstMpiDynaRegCfg->au8LumaWgt[5], pstMpiDynaRegCfg->au8LumaWgt[6], pstMpiDynaRegCfg->au8LumaWgt[7],
                                   pstMpiDynaRegCfg->au8LumaWgt[8], pstMpiDynaRegCfg->au8LumaWgt[9]);
...
static HI_S32 ISP_SharpenRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->unKey.bit1SharpenCfg) {
        ...
        if (pstSharpenStaticRegCfg->bStaticResh) {
            ...
            isp_sharpen_chrr_var_u32_write(pstBeReg, pstSharpenStaticRegCfg->u8chrRSft[0], pstSharpenStaticRegCfg->u8chrRSft[1], pstSharpenStaticRegCfg->u8chrRSft[2],
                                           pstSharpenStaticRegCfg->u8chrRSft[3], pstSharpenStaticRegCfg->u8chrRVarSft);

            isp_sharpen_dhsf_2dcoef0_47_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD0[4], pstSharpenStaticRegCfg->s8hsfCoefD0[5],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD0[6], pstSharpenStaticRegCfg->s8hsfCoefD0[7]);
            ...
            isp_sharpen_dhsf_2dcoef0_811_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD0[8], pstSharpenStaticRegCfg->s8hsfCoefD0[9],
                                                   pstSharpenStaticRegCfg->s8hsfCoefD0[10], pstSharpenStaticRegCfg->s8hsfCoefD0[11]);
            ...
            isp_sharpen_dhsf_2dcoef0_03_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD0[0], pstSharpenStaticRegCfg->s8hsfCoefD0[1],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD0[2], pstSharpenStaticRegCfg->s8hsfCoefD0[3]);
            ...
            isp_sharpen_dhsf_2dcoef1_811_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD1[8], pstSharpenStaticRegCfg->s8hsfCoefD1[9],
                                                   pstSharpenStaticRegCfg->s8hsfCoefD1[10], pstSharpenStaticRegCfg->s8hsfCoefD1[11]);

            isp_sharpen_udhsf_coef_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefUD[0], pstSharpenStaticRegCfg->s8hsfCoefUD[1], pstSharpenStaticRegCfg->s8hsfCoefUD[2],
            ...
            isp_sharpen_chrb_var_u32_write(pstBeReg, pstSharpenStaticRegCfg->u8chrBSft[0], pstSharpenStaticRegCfg->u8chrBSft[1], pstSharpenStaticRegCfg->u8chrBSft[2],
                                           pstSharpenStaticRegCfg->u8chrBSft[3], pstSharpenStaticRegCfg->u8chrBVarSft);

            isp_sharpen_dhsf_2dcoef3_03_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD3[0], pstSharpenStaticRegCfg->s8hsfCoefD3[1], pstSharpenStaticRegCfg->s8hsfCoefD3[2],
            ...
            isp_sharpen_chrg_sft_u32_write(pstBeReg, pstSharpenStaticRegCfg->u8chrGSft[0], pstSharpenStaticRegCfg->u8chrGSft[1], pstSharpenStaticRegCfg->u8chrGSft[2],
                                           pstSharpenStaticRegCfg->u8chrGSft[3]);

            isp_sharpen_dhsf_2dcoef3_47_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD3[4], pstSharpenStaticRegCfg->s8hsfCoefD3[5],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD3[6], pstSharpenStaticRegCfg->s8hsfCoefD3[7]);

            isp_sharpen_dlpf_coef_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8lpfCoefD[0], pstSharpenStaticRegCfg->s8lpfCoefD[1],
                                            pstSharpenStaticRegCfg->s8lpfCoefD[2], pstSharpenStaticRegCfg->u8lpfSftD);

            isp_sharpen_udlpf_coef_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8lpfCoefUD[0], pstSharpenStaticRegCfg->s8lpfCoefUD[1],
                                             pstSharpenStaticRegCfg->s8lpfCoefUD[2], pstSharpenStaticRegCfg->u8lpfSftUD);

            isp_sharpen_dhsf_2dcoef2_47_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD2[4], pstSharpenStaticRegCfg->s8hsfCoefD2[5],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD2[6], pstSharpenStaticRegCfg->s8hsfCoefD2[7]);
            ...
            isp_sharpen_dhsf_2dcoef2_03_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD2[0], pstSharpenStaticRegCfg->s8hsfCoefD2[1],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD2[2], pstSharpenStaticRegCfg->s8hsfCoefD2[3]);

            isp_sharpen_dhsf_2dcoef3_811_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD3[8], pstSharpenStaticRegCfg->s8hsfCoefD3[9],
                                                   pstSharpenStaticRegCfg->s8hsfCoefD3[10], pstSharpenStaticRegCfg->s8hsfCoefD3[11]);
            ...
            isp_sharpen_dhsf_2dcoef2_811_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD2[8], pstSharpenStaticRegCfg->s8hsfCoefD2[9],
                                                   pstSharpenStaticRegCfg->s8hsfCoefD2[10], pstSharpenStaticRegCfg->s8hsfCoefD2[11]);

            isp_sharpen_dhsf_2dcoef1_1215_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD1[12]);

            isp_sharpen_dhsf_2dcoef1_03_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD1[0], pstSharpenStaticRegCfg->s8hsfCoefD1[1],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD1[2], pstSharpenStaticRegCfg->s8hsfCoefD1[3]);
            ...
            isp_sharpen_dhsf_2dcoef1_47_u32_write(pstBeReg, pstSharpenStaticRegCfg->s8hsfCoefD1[4], pstSharpenStaticRegCfg->s8hsfCoefD1[5],
                                                  pstSharpenStaticRegCfg->s8hsfCoefD1[6], pstSharpenStaticRegCfg->s8hsfCoefD1[7]);
...
static HI_S32 ISP_LdciRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    S_ISPBE_REGS_TYPE     *pstBeReg        = HI_NULL;
    ISP_CTX_S             *pstIspCtx       = HI_NULL;
    ISP_LDCI_STAT_S       *pstReadSttBuf   = HI_NULL;
    ISP_LDCI_READ_STT_BUF_S *pstLdciReadSttBuf = HI_NULL;
    ...
    if (pstRegCfgInfo->unKey.bit1LdciCfg) {
        ...
        if (pstIspCtx->bLdciTprFltEn == HI_TRUE) {
            ...
            if (i == 0) {
                pstReadSttBuf = (ISP_LDCI_STAT_S *)ISP_GetLdciReadSttVirAddr(ViPipe, u8ReadBufIdx);
                ISP_CHECK_POINTER(pstReadSttBuf);
                memcpy(pstReadSttBuf, &pstDynaRegCfg->stTprStat, sizeof(ISP_LDCI_STAT_S));
            }

            if ((i + 1) == u8BlkNum) {
                pstLdciReadSttBuf->u8BufIdx = (u8ReadBufIdx + 1) % DIV_0_TO_1(pstLdciReadSttBuf->u8BufNum);
            }

            /* Set ReadStt Addr */
            s32Ret = isp_ldci_read_stt_addr_write(pstViProcReg, pstLdciReadSttBuf->astReadBuf[u8ReadBufIdx].u64PhyAddr);
        ...
        {
            isp_ldci_en_write(pstViProcReg, 0);
        }
        else
        {
        ...
        isp_ldci_cfg_u32_write(pstBeReg, pstDynaRegCfg->bCalcEnable, pstDynaRegCfg->bWrstatEn, bRdstatEn);
...
static HI_S32 ISP_LscRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->unKey.bit1LscCfg) {
        ...
        if (pstUsrRegCfg->bCoefUpdate) {
            ...
            isp_mlsc_mesh_str_write(pstBeReg, pstUsrRegCfg->u16MeshStr);
        ...
        if (bUsrResh) {
            ...
            if (IS_ONLINE_MODE(pstIspCtx->stBlockAttr.enIspRunningMode)\
                || IS_SIDEBYSIDE_MODE(pstIspCtx->stBlockAttr.enIspRunningMode)) {
                if (pstRegCfgInfo->stAlgRegCfg[i].stLscRegCfg.bLut2SttEn == HI_TRUE) { /* online Lut2stt regconfig */
...
static HI_S32 ISP_GammaRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->unKey.bit1GammaCfg) {
        ...
        if (bUsrResh) {
            ...
            isp_gamma_step_u32_write(pstBeReg, pstUsrRegCfg->au8GammaStep[0], pstUsrRegCfg->au8GammaStep[1], pstUsrRegCfg->au8GammaStep[2], pstUsrRegCfg->au8GammaStep[3],
                                     pstUsrRegCfg->au8GammaStep[4], pstUsrRegCfg->au8GammaStep[5], pstUsrRegCfg->au8GammaStep[6], pstUsrRegCfg->au8GammaStep[7]);
...
static HI_S32 ISP_WdrRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->unKey.bit1FsWdrCfg) {
        ...
        if (pstStaticRegCfg->bFirstFrame == HI_TRUE) {
            ...
            isp_wdr_wgtidx_thr_u32_write(pstBeReg, pstDynaRegCfg->u16LongThr, pstDynaRegCfg->u16ShortThr);
...
static HI_S32 ISP_DrcRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->unKey.bit1DrcCfg) {
        ...
        if (pstRegCfgInfo->stAlgRegCfg[i].stDrcRegCfg.bDrcEn == HI_TRUE) {
            isp_drc_dither_en_write(pstBeReg, HI_FALSE);
        } else {
            isp_drc_dither_en_write(pstBeReg, !(pstIspCtx->stHdrAttr.enDynamicRange == DYNAMIC_RANGE_XDR));
        }
        ...
        if (pstStaticRegCfg->bStaticResh) {
            ...
            if (pstStaticRegCfg->bFirstFrame) {
                isp_drc_shp_cfg_u32_write(pstBeReg, pstStaticRegCfg->u8ShpLog, pstStaticRegCfg->u8ShpExp);
                isp_drc_div_denom_log_u32_write(pstBeReg, pstStaticRegCfg->u32DivDenomLog);
                isp_drc_denom_exp_u32_write(pstBeReg, pstStaticRegCfg->u32DenomExp);
                isp_drc_prev_luma_0_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[0]);
                isp_drc_prev_luma_1_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[1]);
                isp_drc_prev_luma_2_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[2]);
                isp_drc_prev_luma_3_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[3]);
                isp_drc_prev_luma_4_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[4]);
                isp_drc_prev_luma_5_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[5]);
                isp_drc_prev_luma_6_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[6]);
                isp_drc_prev_luma_7_u32_write(pstBeReg, pstStaticRegCfg->au32PrevLuma[7]);
        ...
        if (bUsrResh) {
            ...
            isp_drc_flt_cfg_u32_write(pstBeReg, pstUsrRegCfg->u8FltSpaFine, pstUsrRegCfg->u8FltSpaMedium, pstUsrRegCfg->u8FltSpaCoarse,
                                      pstUsrRegCfg->u8FltRngFine, pstUsrRegCfg->u8FltRngMedium, pstUsrRegCfg->u8FltRngCoarse);
            ...
            isp_drc_dp_dtc_u32_write(pstBeReg, pstUsrRegCfg->u16DpDetectThrMin, pstUsrRegCfg->u8DpDetectThrSlo, pstUsrRegCfg->bDpDetectG2RB,
                                     pstUsrRegCfg->bDpDetectRB2RB, pstUsrRegCfg->bDpDetectReplCtr, pstUsrRegCfg->u8DpDetectRngRatio);
            isp_drc_vbi_strength_u32_write(pstBeReg, pstUsrRegCfg->u8VarRngFine, pstUsrRegCfg->u8VarRngMedium, pstUsrRegCfg->u8VarRngCoarse,
                                           pstUsrRegCfg->u8VarSpaFine, pstUsrRegCfg->u8VarSpaMedium, pstUsrRegCfg->u8VarSpaCoarse);
        ...
        if (pstBeBlcCfg->bReshDyna) {
            ...
            if (bIsOfflineMode) {
                if (pstFeBlcCfg->bFeBlcCfgChanged) {
                    blc0RegValue = (pstBeBlcCfg->u16DgBlcCfgSave[1] << 16) + pstBeBlcCfg->u16DgBlcCfgSave[0];
                    blc1RegValue = (pstBeBlcCfg->u16DgBlcCfgSave[3] << 16) + pstBeBlcCfg->u16DgBlcCfgSave[2];
                } else {
                    blc0RegValue = (pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[1] << 16) + pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[0];
                    blc1RegValue = (pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[3] << 16) + pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[2];
                }
                for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
                    pstBeBlcCfg->u16DgBlcCfgSave[j] = pstBeBlcCfg->stDgBlc.stUsrRegCfg.au16Blc[j];
                }
                isp_dg_blc_offset1_u32_write(pstBeReg, (blc0RegValue >> 16), (blc0RegValue & 0xffff));
                isp_dg_blc_offset2_u32_write(pstBeReg, (blc1RegValue >> 16), (blc1RegValue & 0xffff));
            } else {
            ...
            if (bIsOfflineMode) {
                if (pstFeBlcCfg->bFeBlcCfgChanged) {
                    wdrBlc00  = pstBeBlcCfg->u16WdrBlcCfgSave[0][0];
                    wdrBlc01  = pstBeBlcCfg->u16WdrBlcCfgSave[0][1];
                    wdrBlc02  = pstBeBlcCfg->u16WdrBlcCfgSave[0][2];
                    wdrBlc03  = pstBeBlcCfg->u16WdrBlcCfgSave[0][3];
                    wdrBlc10  = pstBeBlcCfg->u16WdrBlcCfgSave[1][0];
                    wdrBlc11  = pstBeBlcCfg->u16WdrBlcCfgSave[1][1];
                    wdrBlc12  = pstBeBlcCfg->u16WdrBlcCfgSave[1][2];
                    wdrBlc13  = pstBeBlcCfg->u16WdrBlcCfgSave[1][3];
                    wdrBlcOut = pstBeBlcCfg->u16WdrBlcOutCfgSave;
                } else {
                    wdrBlc00  = pstBeBlcCfg->stWdrBlc[0].stUsrRegCfg.au16Blc[0];
                    wdrBlc01  = pstBeBlcCfg->stWdrBlc[0].stUsrRegCfg.au16Blc[1];
                    wdrBlc02  = pstBeBlcCfg->stWdrBlc[0].stUsrRegCfg.au16Blc[2];
                    wdrBlc03  = pstBeBlcCfg->stWdrBlc[0].stUsrRegCfg.au16Blc[3];

                    wdrBlc10  = pstBeBlcCfg->stWdrBlc[1].stUsrRegCfg.au16Blc[0];
                    wdrBlc11  = pstBeBlcCfg->stWdrBlc[1].stUsrRegCfg.au16Blc[1];
                    wdrBlc12  = pstBeBlcCfg->stWdrBlc[1].stUsrRegCfg.au16Blc[2];
                    wdrBlc13  = pstBeBlcCfg->stWdrBlc[1].stUsrRegCfg.au16Blc[3];
                    wdrBlcOut = pstBeBlcCfg->stWdrBlc[0].stUsrRegCfg.u16OutBlc;
                }
                isp_wdr_f0_inblc0_u32_write(pstBeReg, wdrBlc01, wdrBlc00);
                isp_wdr_f0_inblc1_u32_write(pstBeReg, wdrBlc03, wdrBlc02);
                isp_wdr_f1_inblc0_u32_write(pstBeReg, wdrBlc11, wdrBlc10);
                isp_wdr_f1_inblc1_u32_write(pstBeReg, wdrBlc13, wdrBlc12);
                isp_wdr_outblc_write(pstBeReg, wdrBlcOut);
                for (j = 0; j < ISP_BAYER_CHN_NUM; j++) {
...
static HI_S32 ISP_SystemRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    if (pstRegCfgInfo->stAlgRegCfg[i].stFpnRegCfg.stDynaRegCfg.u32IspFpnCalibCorr == 1) {  // fpn calib mode
...
static HI_S32 ISP_BeCurRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    isp_ca_isoratio_write(pstBeReg, pstAlgRegCfg->stCaRegCfg.stDynaRegCfg.u16CaISORatio);
    ...
    isp_dehaze_gstrth_u32_write(pstBeReg, pstAlgRegCfg->stDehazeRegCfg.stDynaRegCfg.u8Strength);
    return HI_SUCCESS;
}
...
static HI_S32 ISP_BeLastRegConfig(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    isp_ca_llhc_ratio_u32_write(pstBeReg, pstLut2SttSyncCfg->stCaSyncCfg.s16CaLumaRatioHigh, pstLut2SttSyncCfg->stCaSyncCfg.s16CaLumaRatioLow);
    isp_ca_yraratio_u32_write(pstBeReg, pstLut2SttSyncCfg->stCaSyncCfg.s16CaYLumaRatio);
    isp_ca_isoratio_write(pstBeReg, pstLut2SttSyncCfg->stCaSyncCfg.u16CaIsoRatio);
    ...
    if (pstRegCfgInfo->stAlgRegCfg[i].stDehazeRegCfg.bLut2SttEn == HI_TRUE) {
        ...
        isp_dehaze_gstrth_u32_write(pstBeReg, pstLut2SttSyncCfg->stDehazeSyncCfg.u8Strength);
    }
...
static HI_S32 ISP_SaveBeSyncReg(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo, HI_U8 i)
{
    ...
    pstLut2SttSyncCfg->stCaSyncCfg.u16CaIsoRatio      = pstAlgRegCfg->stCaRegCfg.stDynaRegCfg.u16CaISORatio;
...
HI_S32 ISP_AlgEnExit(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stIspParaRec.bInit == HI_FALSE) {
        return HI_SUCCESS;
    }
...
static HI_S32 ISP_BeRegsConfigInit(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfgInfo)
{
    ...
    if (pstIspCtx->u32FrameCnt == 0) {   // when mode switch,not need write besync buffer
...
HI_S32 ISP_SyncCfgSet(VI_PIPE ViPipe)
{
    ...
    while (s32PipeSt <= s32PipeEd) {
        ...
        pstRegCfg->stSyncCfgNode.stAWBRegCfg.enBEAWBSwitch = pstRegCfg->stRegCfg.stAlgRegCfg[0].stAwbRegCfg.stAwbRegUsrCfg.enBEAWBSwitch;
...
HI_S32 ISP_RegCfgCtrl(VI_PIPE ViPipe)
{
    ...
    if (IS_ONLINE_MODE(pstIspCtx->stBlockAttr.enIspRunningMode)) {
        pstIspCtx->stSpecialOpt.abBeOnSttUpdate[0] = HI_TRUE;
...
static HI_S32 ISP_LutApbRegconfig(VI_PIPE ViPipe, ISP_REGCFG_S *pstRegCfg)
{
    HI_U8  i;
    HI_U16 j;
    HI_U32 u32SharpenLutData;
    S_ISPBE_REGS_TYPE  *pstBeReg     = HI_NULL;
    HI_U32 u32BnrLutData;
    ...
    for (i = 0; i < pstRegCfg->stRegCfg.u8CfgNum; i++) {
        ...
        for (j = 0; j < GAMMA_REG_NODE_NUM; j++) {
            isp_gamma_lut_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stGammaCfg.stUsrRegCfg.au16GammaLUT[j]);
        }
        ...
        for (j = 0; j < SHRP_GAIN_LUT_SIZE; j++) {
            u32SharpenLutData = (pstRegCfg->stRegCfg.stAlgRegCfg[i].stSharpenRegCfg.stDynaRegCfg.stMpiDynaRegCfg.u16hfGainD[j] << 12) +
                                (pstRegCfg->stRegCfg.stAlgRegCfg[i].stSharpenRegCfg.stDynaRegCfg.stMpiDynaRegCfg.u16mfGainD[j]);
            isp_sharpen_mhfgaind_wdata_u32_write(pstBeReg, u32SharpenLutData);

            u32SharpenLutData = (pstRegCfg->stRegCfg.stAlgRegCfg[i].stSharpenRegCfg.stDynaRegCfg.stMpiDynaRegCfg.u16hfGainUD[j] << 12) +
                                (pstRegCfg->stRegCfg.stAlgRegCfg[i].stSharpenRegCfg.stDynaRegCfg.stMpiDynaRegCfg.u16mfGainUD[j]);

            isp_sharpen_mhfgainud_wdata_u32_write(pstBeReg, u32SharpenLutData);
        ...
        for (j = 0; j < PREGAMMA_NODE_NUM; j++) {
            isp_drc_pregamma_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stPreGammaCfg.stDynaRegCfg.au16PreGammaLUT[j]);
        }
        isp_drc_lut_update1_u32_write(pstBeReg, HI_TRUE);

        // bnr
        isp_bnr_lmt_even_waddr_u32_write(pstBeReg, 0);
        for (j = 0; j < HI_ISP_BAYERNR_LMTLUT_EVEN_NUM; j++) {
            u32BnrLutData = (pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8JnlmLimitSEvenLut[j] << 8) +
                            (pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8JnlmLimitEvenLut[j]  & 0xff);
            isp_bnr_lmt_even_wdata_u32_write(pstBeReg, u32BnrLutData);
        }

        isp_bnr_lmt_odd_waddr_u32_write(pstBeReg, 0);
        for (j = 0; j < HI_ISP_BAYERNR_LMTLUT_ODD_NUM; j++) {
            u32BnrLutData = (pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8JnlmLimitSOddLut[j] << 8) +
                            (pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8JnlmLimitOddLut[j]  & 0xff);

            isp_bnr_lmt_odd_wdata_u32_write(pstBeReg, u32BnrLutData);
        }


        isp_bnr_cor_even_waddr_u32_write(pstBeReg, 0);
        isp_bnr_gcor_even_waddr_u32_write(pstBeReg, 0);
        for (j = 0; j < HI_ISP_BAYERNR_LUT_EVEN_LENGTH; j++) {

            isp_bnr_cor_even_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au16JnlmCoringLowEvenLut[j]);
            isp_bnr_gcor_even_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8GcoringEvenLut[j]);
        }


        isp_bnr_cor_odd_waddr_u32_write(pstBeReg, 0);
        isp_bnr_gcor_odd_waddr_u32_write(pstBeReg, 0);
        for (j = 0; j < HI_ISP_BAYERNR_LUT_ODD_LENGTH; j++) {
            isp_bnr_cor_odd_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au16JnlmCoringLowOddLut[j]);
            isp_bnr_gcor_odd_wdata_u32_write(pstBeReg, pstRegCfg->stRegCfg.stAlgRegCfg[i].stBnrRegCfg.stDynaRegCfg.au8GcoringOddLut[j]);
        }
