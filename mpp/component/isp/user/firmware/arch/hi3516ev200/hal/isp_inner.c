...
HI_S32 ISP_SetRGBIRAttr(VI_PIPE ViPipe, const ISP_RGBIR_ATTR_S *pstRGBIRAttr)
{
    ...
    ISP_CHECK_OPEN(ViPipe);
    ISP_CHECK_MEM_INIT(ViPipe);
    if ((pstRGBIRAttr->bEnable == HI_TRUE) && (IS_WDR_MODE(pstIspCtx->stWdrAttr.enWDRMode))) {
    ...
    for (i = 0; i < ISP_RGBIR_CVTMATRIX_NUM; i++) {
        ...
        } else if (pstRGBIRAttr->enIRCvtMatMode == ISP_IR_CVTMAT_MODE_MONO) {
    ...
    if (ioctl(g_as32IspFd[ViPipe], ISP_SET_RGBIR_FORMAT, &pstRGBIRAttr->enInPattern) != HI_SUCCESS) {
