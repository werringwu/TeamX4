...
HI_S32 HI_ISP_Init(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stIspParaRec.bInit == HI_TRUE) {
    ...
    if (pstSnsDft->stSensorMode.bValidDngRawFormat == HI_TRUE) {
...
HI_S32  HI_ISP_YUV_RunOnce(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stIspParaRec.bRun == HI_TRUE) {
    ...
    if (pstIspCtx->stIspParaRec.bRunEn == HI_FALSE) {
        MUTEX_UNLOCK(pstIspCtx->stLock);
        return HI_SUCCESS;
    }
...
HI_S32 HI_MPI_ISP_SetDCFInfo(VI_PIPE ViPipe, const ISP_DCF_INFO_S *pstIspDCF)
{
    ...
    if (pstIspCtx->stIspTransInfo.stUpdateInfo.u64PhyAddr == 0) {
        ...
        if (pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo == HI_NULL) {
