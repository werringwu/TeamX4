...
HI_S32 ISP_SensorRegCallBack(VI_PIPE ViPipe, ISP_SNS_ATTR_INFO_S *pstSnsAttrInfo, ISP_SENSOR_REGISTER_S *pstRegister)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_global_init != HI_NULL) {
...
HI_S32 ISP_SensorUpdateAll(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_default != HI_NULL) {
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_black_level != HI_NULL) {
...
HI_S32 ISP_SensorInit(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_init != HI_NULL) {
...
HI_S32 ISP_SensorExit(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_exit != HI_NULL) {
...
HI_S32 ISP_SensorSetWDRMode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_wdr_mode != HI_NULL) {
        if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_wdr_mode(ViPipe, u8Mode) != HI_SUCCESS) {
            return HI_FAILURE;
        }
    }
...
HI_S32 ISP_SensorSetImageMode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSnsImageMode)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_image_mode != HI_NULL) {
...
HI_S32 ISP_SensorSetPixelDetect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_pixel_detect != HI_NULL) {
...
HI_S32 ISP_SensorUpdateSnsReg(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_sns_reg_info != HI_NULL) {
