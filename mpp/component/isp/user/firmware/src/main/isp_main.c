...
HI_S32 ISP_UpdateInfo(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < ISP_MAX_ALGS_NUM; i++) {
        if (pstIspCtx->astAlgs[i].bUsed) {
            if (pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl != HI_NULL) {
...
HI_S32 ISP_UpdateAttachInfo(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stAttachInfoCtrl.pstAttachInfo == HI_NULL) {
...
HI_S32 ISP_UpdateColorGamutinfo(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stGamutInfoCtrl.pstColorGamutInfo == HI_NULL) {
...
HI_S32 ISP_SwitchWDR(VI_PIPE ViPipe)
{
    ...
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "pipe:%d Config ISP Mode Switch Failed with ec %#x!\n", ViPipe, s32Ret);
        return s32Ret;
    }
...
static HI_S32 ISP_SwitchModeProcess(VI_PIPE ViPipe)
{
    ...
    if ((pstIspCtx->bChangeIspRes == HI_TRUE) || (pstIspCtx->bChangeImageMode == HI_TRUE)) {
        ISP_AlgsCtrl(pstIspCtx->astAlgs, ViPipe, ISP_CHANGE_IMAGE_MODE_SET, (HI_VOID *)&stIspImageMode);
    }
    ISP_AlgsCtrl(pstIspCtx->astAlgs, ViPipe, ISP_AE_FPS_BASE_SET,
                 (HI_VOID *)&pstIspCtx->stSnsImageMode.f32Fps);
...
HI_S32 ISP_SnapPreProcess(VI_PIPE ViPipe)
{
    ...
    if ((enSnapPipeMode == ISP_SNAP_PREVIEW) || (enSnapPipeMode == ISP_SNAP_PREVIEW_PICTURE)) {
        if (enSnapType == SNAP_TYPE_PRO) {
            ...
            if (pstIspCtx->stLinkage.bProTrigger == HI_TRUE) {
...
HI_S32 ISP_StitchSyncExit(VI_PIPE ViPipe)
{
    ...
    if (( pstIspCtx->stStitchAttr.bStitchEnable == HI_TRUE) && (pstIspCtx->stStitchAttr.bMainPipe != HI_TRUE )) {
        for (k = 0; k < pstIspCtx->stStitchAttr.u8StitchPipeNum; k++) {
            ...
            while ((pstIspCtx->stStitchAttr.bMainPipe == HI_TRUE) && (pstIspCtx->bMemInit != HI_FALSE )) {
                usleep(10);
            }
        }
    }
...
HI_S32 ISP_StitchSyncRun(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stStitchAttr.bStitchEnable == HI_TRUE ) {
        if (pstIspCtx->stStitchAttr.bMainPipe == HI_TRUE ) {
            for (k = 0; k < pstIspCtx->stStitchAttr.u8StitchPipeNum; k++) {
                ViPipeId = pstIspCtx->stStitchAttr.as8StitchBindId[k];
                ISP_GET_CTX(ViPipeId, pstIspCtxS);

                if (pstIspCtxS->bMemInit != HI_TRUE) {
                    return HI_FAILURE;
                }

                if (pstIspCtxS->stIspParaRec.bInit != HI_TRUE) {
                    return HI_FAILURE;
                }
            }
        }
...
HI_S32 ISP_Run(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stLinkage.bStatReady == HI_TRUE) {
    ...
    if (((pstIspCtx->u8ProFrmCnt > 0) && (pstIspCtx->u8ProFrmCnt < pstIspCtx->stProParam.u8ProFrameNum + 4)) ||
        ( pstIspCtx->stLinkage.bProTrigger == HI_TRUE)) {
...
HI_S32 ISP_Exit(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->bISPYuvMode == HI_FALSE) {
