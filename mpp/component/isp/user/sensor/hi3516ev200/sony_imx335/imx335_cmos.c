...
static HI_U32   g_u32Imx335AGain[ISP_MAX_PIPE_NUM] = {[0 ...(ISP_MAX_PIPE_NUM - 1)] = 1024};
static HI_U32   g_u32Imx335DGain[ISP_MAX_PIPE_NUM] = {[0 ...(ISP_MAX_PIPE_NUM - 1)] = 1024};
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {
        ...
        case IMX335_4M_25FPS_10BIT_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 15)) {
                u32MaxFps = 25;
                u32Lines = IMX335_VMAX_4M_25FPS_10BIT_WDR * u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
    ...
    if (WDR_MODE_NONE == pstSnsState->enWDRMode) {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = IMX335_LOW_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_HIG_4BITS(pstSnsState->au32FL[0]);

    } else {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = IMX335_LOW_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = IMX335_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = IMX335_HIG_4BITS(pstSnsState->au32FL[0]);
    }
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
        ...
        */
        g_astimx335State[ViPipe].u32RHS1_MAX = g_astimx335State[ViPipe].u32BRL * 2;
        g_astimx335State[ViPipe].u32deltaRHS1 = (u32Lines * 2) - (g_astimx335State[ViPipe].u32BRL * 2) - 2;
    } else {
        pstSnsState->u32FLStd = u32Lines;
    }
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
        ...
        u32FullLines = (u32FullLines > (2 * 2 * u32Fll)) ? (2 * 2 * u32Fll) : u32FullLines;

        if (0 != (u32FullLines % 4)) {
            u32FullLines = u32FullLines + (4 - (u32FullLines % 4));     // Because FSC value an integer multiple of 8
        }
        pstSnsState->au32FL[0] = u32FullLines;

        g_astimx335State[ViPipe].u32RHS1_MAX = g_astimx335State[ViPipe].u32BRL * 2;
        g_astimx335State[ViPipe].u32deltaRHS1 = u32FullLines - (g_astimx335State[ViPipe].u32BRL * 2) - 2;
    } else {
    ...
    if (WDR_MODE_NONE == pstSnsState->enWDRMode) {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = IMX335_LOW_8BITS(pstSnsState->au32FL[0]);   // VMAX
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_HIG_4BITS(pstSnsState->au32FL[0]);
    } else {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = IMX335_LOW_8BITS(pstSnsState->au32FL[0] >> 1);  // VMAX
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = IMX335_MID_8BITS(pstSnsState->au32FL[0] >> 1);
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = IMX335_HIG_4BITS(pstSnsState->au32FL[0] >> 1);
    }
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
        if (bFirst[ViPipe]) { /* short exposure */
            ...
            if (0 == u16ShortExpMod8) {
                ...
                if (u16ShortExpMod4 == u16ShortExpMod8) {
                    u32SHR1 = 18;
                    u32RHS1 = 18 + u32ShortIntTime - u16ShortExpMod8;
                } else {
                    u32SHR1 = 22;
                    u32RHS1 = 22 + u32ShortIntTime - u16ShortExpMod4;
                }
            }
            ...
            pstSnsState->astRegsInfo[0].astI2cData[0].u32Data =  IMX335_LOW_8BITS(u32SHR0);    // SHR0
            pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = IMX335_MID_8BITS(u32SHR0);
            pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_HIG_4BITS(u32SHR0);

            pstSnsState->astRegsInfo[0].astI2cData[5].u32Data =  IMX335_LOW_8BITS(u32SHR1);    // SHR1
            pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_MID_8BITS(u32SHR1);
            pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_HIG_4BITS(u32SHR1);

            pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =  IMX335_LOW_8BITS(u32RHS1);    // RHS1
            pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = IMX335_MID_8BITS(u32RHS1);
            pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = IMX335_HIG_4BITS(u32RHS1);
        ...
        pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_HIG_4BITS(u32Value);
    }
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (bFirstGain[ViPipe]) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[3].u32Data = IMX335_LOW_8BITS(u32Tmp);
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = IMX335_MID_8BITS(u32Tmp);
            bFirstGain[ViPipe] = HI_FALSE;
        } else {
            u32Tmp = u32Again + u32Dgain;

            pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_LOW_8BITS(u32Tmp);
            pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_MID_8BITS(u32Tmp);
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if ((WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)) {
        ...
        */
        ...
        u16LimitValue = (IMX335_VMAX_4M_30FPS_10BIT_WDR*2 - g_astimx335State[ViPipe].u32BRL*2 - 2) - 8;
        u16LimitValue = 300;
        ...
        if (ISP_FSWDR_LONG_FRAME_MODE == genFSWDRMode[ViPipe]) { /* long frame mode enable */
            ...
            if (pstSnsState->au32WDRIntTime[0] == u32ShortTimeMinLimit && au32Ratio[0] == 0x40) {
                ...
                u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
                u32IntTimeMaxTmp = (u32IntTimeMaxTmp > (g_astimx335State[ViPipe].u32RHS1_MAX - 18)) ? (g_astimx335State[ViPipe].u32RHS1_MAX - 18) : u32IntTimeMaxTmp;
                if (u32IntTimeMaxTmp > u32LastIntTimeMaxTmp) {
                    u32delta = u32IntTimeMaxTmp - u32LastIntTimeMaxTmp;
                    if (u32delta > g_astimx335State[ViPipe].u32deltaRHS1) {
                        u32IntTimeMaxTmp = u32LastIntTimeMaxTmp + (g_astimx335State[ViPipe].u32deltaRHS1 - (g_astimx335State[ViPipe].u32deltaRHS1 % 8));
                    } else {
                    }
                } else if (u32LastIntTimeMaxTmp > u32IntTimeMaxTmp) {
                    u32delta = u32LastIntTimeMaxTmp - u32IntTimeMaxTmp;
                    if (u32delta > g_astimx335State[ViPipe].u32deltaRHS1) {
                        u32IntTimeMaxTmp = u32LastIntTimeMaxTmp - (g_astimx335State[ViPipe].u32deltaRHS1 - (g_astimx335State[ViPipe].u32deltaRHS1 % 8));
                    } else {
                    }
            ...
            if (u32IntTimeMaxTmp >= u32LastIntTimeMaxTmp)
            {
                ...
                {
                    u32IntTimeMaxTmp = u32LastIntTimeMaxTmp + u16LimitValue;
                }
            }
            else if (u32LastIntTimeMaxTmp > u32IntTimeMaxTmp)
            {
                ...
                if (u32delta > u16LimitValue)
                {
                    if (pstSnsState->au32WDRIntTime[0] >  (u16LimitValue + 4))
                    {
                         u32ShortTimeMinLimit = pstSnsState->au32WDRIntTime[0] - u16LimitValue;
                    }

                    u32IntTimeMaxTmp = pstSnsState->au32WDRIntTime[0];
            ...
            u32IntTimeMaxTmp = (0 == u32IntTimeMaxTmp) ? 1 : u32IntTimeMaxTmp;
            u32LastIntTimeMaxTmp = u32IntTimeMaxTmp;
            *pu32LFMaxIntTime = g_astimx335State[ViPipe].u32RHS1_MAX - 18;
        }
    } else {
    }
...
static HI_S32 cmos_get_isp_black_level(VI_PIPE ViPipe, ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
    ...
    if (HI_TRUE == pstBlackLevel->bUpdate) {
        if (g_u32Imx335AGain[ViPipe] >= 32381) {
            ...
            } else if ((g_u32Imx335DGain[ViPipe] >= 12288) && (g_u32Imx335DGain[ViPipe] < 16384)) {
            ...
            } else if ((g_u32Imx335DGain[ViPipe] >= 16384) && (g_u32Imx335DGain[ViPipe] < 20480)) {
            ...
            } else if ((g_u32Imx335DGain[ViPipe] >= 20480) && (g_u32Imx335DGain[ViPipe] < 24576)) {
            ...
            } else if ((g_u32Imx335DGain[ViPipe] >= 24576) && (g_u32Imx335DGain[ViPipe] < 28672)) {
            ...
            } else if ((g_u32Imx335DGain[ViPipe] >= 28672) && (g_u32Imx335DGain[ViPipe] < 31768)) {
        ...
        if (WDR_MODE_NONE == pstSnsState->enWDRMode) {
            ...
            for (i = 0; i < 4; i++) {
                pstBlackLevel->au16BlackLevel[i] = 200; /* 10bit 50, 12bit 200 */
            }
        } else {
            for (i = 0; i < 4; i++) {
                pstBlackLevel->au16BlackLevel[i] = 200; /* 10bit 50, 12bit 200 */
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
        ...
        if (IMX335_5M_30FPS_12BIT_LINEAR_MODE == pstSnsState->u8ImgMode) {
            u32FullLines_5Fps = (IMX335_VMAX_5M_30FPS_12BIT_LINEAR * 30) / 5;
        } else {
            return;
        }
    }
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        IMX335_write_register(ViPipe, IMX335_GAIN_LONG_LOW, 0x00); // gain
        IMX335_write_register(ViPipe, IMX335_GAIN_LONG_HIGH, 0x00);

        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_L, IMX335_LOW_8BITS(u32FullLines_5Fps));    // VMAX
        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_M, IMX335_MID_8BITS(u32FullLines_5Fps));
        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_H, IMX335_HIG_4BITS(u32FullLines_5Fps));

        IMX335_write_register(ViPipe, IMX335_SHR0_LOW, IMX335_LOW_8BITS(u32MaxIntTime_5Fps));
        IMX335_write_register(ViPipe, IMX335_SHR0_MIDDLE, IMX335_MID_8BITS(u32MaxIntTime_5Fps));
        IMX335_write_register(ViPipe, IMX335_SHR0_MIDDLE, IMX335_HIG_4BITS(u32MaxIntTime_5Fps));
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > IMX335_FULL_LINES_MAX) ? IMX335_FULL_LINES_MAX : pstSnsState->u32FLStd;
        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_L, IMX335_LOW_8BITS(pstSnsState->u32FLStd));
        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_M, IMX335_MID_8BITS(pstSnsState->u32FLStd));
        IMX335_write_register(ViPipe, IMX335_VMAX_ADDR_H, IMX335_HIG_4BITS(pstSnsState->u32FLStd));
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((HI_FALSE == pstSnsState->bSyncInit) || (HI_FALSE == pstSnsRegsInfo->bConfig)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx335BusInfo[ViPipe].s8I2cDev;
        ...
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX335_SHR0_HIGH;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;        // Long Gain
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX335_GAIN_LONG_LOW;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX335_GAIN_LONG_HIGH;

        // Vmax
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX335_VMAX_ADDR_L;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX335_VMAX_ADDR_M;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX335_VMAX_ADDR_H;
        ...
        if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX335_SHR1_LOW;    // SHR1
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX335_SHR1_MIDDLE;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX335_SHR1_HIGH;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX335_VMAX_ADDR_L;    // VMAX wdr
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX335_VMAX_ADDR_M;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX335_VMAX_ADDR_H;

            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX335_RHS1_LOW;    // RHS1
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX335_RHS1_MIDDLE;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX335_RHS1_HIGH;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX335_GAIN_SHORT_LOW;    // Short Gain
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX335_GAIN_SHORT_HIGH;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
        if (IMX335_RES_IS_4M_10BIT_WDR_EX(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
        ...
        } else if (IMX335_RES_IS_5M_10BIT_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height )) {
            u8SensorImageMode               = IMX335_5M_30FPS_10BIT_WDR_MODE;
            g_astimx335State[ViPipe].u32BRL = 1984 * 2;
            pstSnsState->u32FLStd           = IMX335_VMAX_5M_30FPS_10BIT_WDR;
        } else if (IMX335_RES_IS_4M_10BIT_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
...
static HI_S32 IMX335_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
