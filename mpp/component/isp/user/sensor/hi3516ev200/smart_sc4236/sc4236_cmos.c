...
******************************************************************************/
#if !defined(__SC4236_CMOS_H_)
#define __SC4236_CMOS_H_
...
ISP_SNS_STATE_S *g_pastSc4236[ISP_MAX_PIPE_NUM] = {HI_NULL};
...
#define SC4236_SENSOR_RESET_CTX(dev)         (g_pastSc4236[dev] = HI_NULL)
...
#define SC4236_INCREASE_LINES                 (0) /* make real fps less than stand fps because NVR require */
...
{
    HI_U32 u32FullLines;
    HI_U32 u32VMAX = SC4236_VMAX_1296P30_LINEAR;
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    ...
    switch (pstSnsState->u8ImgMode) {
        case SC4236_SENSOR_1296P_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 5)) { /* Minimum 5fps */
                u32VMAX = SC4236_VMAX_1296P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
    ...
    u32VMAX = (u32VMAX > SC4236_FULL_LINES_MAX) ? SC4236_FULL_LINES_MAX : u32VMAX;
    ...
    pstSnsState->u32FLStd = (u32FullLines > SC4236_FULL_LINES_MAX) ? SC4236_FULL_LINES_MAX : u32FullLines;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = LOW_8BITS((pstSnsState->au32FL[0] + 4) >> 1);
    pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = HIGH_8BITS((pstSnsState->au32FL[0] + 4) >> 1);
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    u32Val = (u32IntTime > SC4236_FULL_LINES_MAX) ? SC4236_FULL_LINES_MAX : u32IntTime;
...
static HI_U32 again_table[64] = {
    ...
    0x1f10, 0x1f11, 0x1f12, 0x1f13, 0x1f14, 0x1f15, 0x1f16, 0x1f17, 0x1f18, 0x1f19, 0x1f1a, 0x1f1b, 0x1f1c, 0x1f1d,
    0x1f1e, 0x1f1f
};

static HI_U32 dgain_table[80] = {
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    } else if ((u8Reg0x3e08 >= 0x07) && (u8Reg0x3e08 < 0x0f)) { /* 0x0710~0x0f10  2x<=gain<4x */
        pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = 0x50;
        pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = 0x23;
        pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = 0x80;
        pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = 0xf6;
    } else if ((0x0f <= u8Reg0x3e08) && (0x1f > u8Reg0x3e08)) { /* 0x0f10~0x1f10  4x<=gain<8x */
        pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = 0x50;
        pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = 0x23;
        pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = 0x80;
        pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = 0xf6;
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        ...
        sc4236_write_register(ViPipe, SC4236_EXP_L_ADDR,  LOWER_4BITS(u32MaxIntTime_5Fps) << 4);
        sc4236_write_register(ViPipe, SC4236_EXP_M_ADDR,  HIGHER_8BITS(u32MaxIntTime_5Fps));
        sc4236_write_register(ViPipe, SC4236_EXP_H_ADDR,  HIGHER_4BITS(u32MaxIntTime_5Fps));
        sc4236_write_register(ViPipe, SC4236_VMAX_L_ADDR, LOW_8BITS(u32FullLines_5Fps));
        sc4236_write_register(ViPipe, SC4236_VMAX_H_ADDR, HIGH_8BITS(u32FullLines_5Fps));
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > SC4236_FULL_LINES_MAX * 2) ? (SC4236_FULL_LINES_MAX * 2) : pstSnsState->u32FLStd;
        sc4236_write_register(ViPipe, SC4236_VMAX_L_ADDR, (pstSnsState->u32FLStd & 0x00F) << 4);
        sc4236_write_register(ViPipe, SC4236_VMAX_H_ADDR, (pstSnsState->u32FLStd & 0xFF0) >> 4);
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunSc4236BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = sc4236_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = sc4236_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = sc4236_data_byte;
        }

        // Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = SC4236_EXP_L_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = SC4236_EXP_M_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = SC4236_EXP_H_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = SC4236_AGAIN_L_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = SC4236_AGAIN_H_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = SC4236_DGAIN_L_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = SC4236_DGAIN_H_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = SC4236_VMAX_L_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = SC4236_VMAX_H_ADDR;
        ...
        pstSnsState->astRegsInfo[0].astI2cData[10].u8IntPos      = 1;   /* FrameEnd interrupt */
        pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = 0x3314; /* preprecharge */
        pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = 0x330e; /* preprecharge */
        ...
        if (HI_TRUE == (pstSnsState->astRegsInfo[0].astI2cData[10].bUpdate\
        ...
        if (HI_TRUE == (pstSnsState->astRegsInfo[0].astI2cData[12].bUpdate\
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (SC4236_RES_IS_1296P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                u8SensorImageMode     = SC4236_SENSOR_1296P_30FPS_LINEAR_MODE;
...
static HI_S32 sc4236_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, SC4236_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC4236_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC4236_ID);
    if (s32Ret != HI_SUCCESS) {
