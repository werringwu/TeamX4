...
#define GC2053_SENSOR_1080P_30FPS_FRAME_WDR_2to1_MODE (1)
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {
        ...
        case GC2053_SENSOR_1080P_30FPS_FRAME_WDR_2to1_MODE:
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
        if (bFirst[ViPipe]) { /* short exposure */
            ...
            pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = (u32Val & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = ((u32Val & 0xFF00) >> 8);
...
static HI_S32 cmos_get_awb_default(VI_PIPE ViPipe, AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
    ...
    pstAwbSnsDft->au16GainOffset[0] = 404; // R
    pstAwbSnsDft->au16GainOffset[1] = 256; // G
    pstAwbSnsDft->au16GainOffset[2] = 256; // G
    pstAwbSnsDft->au16GainOffset[3] = 509; // B
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        if ( pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr  = GC2053_EXPTIME_ADDR_L;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum  = 2;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr  = GC2053_EXPTIME_ADDR_H;
        }
    ...
    if ((pstSensorImageMode->u16Width <= 1920) && (pstSensorImageMode->u16Height <= 1080)) {
        ...
        if (pstSensorImageMode->f32Fps <= 30) {
			if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
				...
				u8SensorImageMode     = GC2053_SENSOR_1080P_30FPS_FRAME_WDR_2to1_MODE;
            	pstSnsState->u32FLStd = GC2053_VMAX_1080P30_LINEAR;
			} else {
...
static HI_S32 gc2053_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
