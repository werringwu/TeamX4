...
HI_S32 gc2053_GetQuickStartFlag(VI_PIPE ViPipe, HI_U32 *quickStartFlag)
{
    HI_S32 s32Ret = 0;
    ISP_MOD_PARAM_S stModParam;
    s32Ret = HI_MPI_ISP_GetModParam(&stModParam);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "gc2053 Get QuickStart Error\n");
        return s32Ret;
    }
...
int gc2053_read_register(VI_PIPE ViPipe, int addr)
{
    HI_S32 s32RegVal = 0;
    if (g_fd[ViPipe] < 0) {
        ISP_TRACE(HI_DBG_ERR, "gc2053_read_register fd not opened!\n");
        return HI_FAILURE;
    }

    HI_S32 s32Ret = 0;
    HI_U32 u32RegWidth = gc2053_addr_byte;
    HI_U32 u32DataWidth = gc2053_data_byte;
    HI_U8 aRecvbuf[4];



#ifdef LOSCFG_HOST_TYPE_HIBVT
    HI_U32 u32SnsI2cAddr = (gc2053_i2c_addr >> 1);
...
int gc2053_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
    ...
    if (quickStartFlag == 0) {
        ...
        case 1:
            ...
            default:
                ...
                g_pastGc2053[ViPipe]->bInit = HI_FALSE;
                break;
        }
        for (i = 0; i < g_pastGc2053[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
            gc2053_write_register(ViPipe, g_pastGc2053[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastGc2053[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
        }
    }
    g_pastGc2053[ViPipe]->bInit = HI_TRUE;
    return;
}
