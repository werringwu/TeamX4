...
ISP_SNS_STATE_S *g_pastSc2231[ISP_MAX_PIPE_NUM] = {HI_NULL};
...
#define SC2231_SENSOR_RESET_CTX(dev)         (g_pastSc2231[dev] = HI_NULL)
...
#define SC2231_INCREASE_LINES                 (0) /* make real fps less than stand fps because NVR require */
...
{
    HI_U32 u32FullLines;
    HI_U32 u32VMAX = SC2231_VMAX_1080P30_LINEAR;
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    ...
    switch (pstSnsState->u8ImgMode) {
        case SC2231_SENSOR_1080P_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 5)) { /* Minimum 5fps */
                u32VMAX = SC2231_VMAX_1080P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
    ...
    u32VMAX = (u32VMAX > SC2231_FULL_LINES_MAX) ? SC2231_FULL_LINES_MAX : u32VMAX;
    ...
    pstSnsState->u32FLStd = (u32FullLines > SC2231_FULL_LINES_MAX) ? SC2231_FULL_LINES_MAX : u32FullLines;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32FullLines = (u32FullLines > SC4236_FRAME_RATE_MIN) ? SC4236_FRAME_RATE_MIN : u32FullLines;
    pstSnsState->au32FL[0] = u32FullLines;
    pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = HIGH_8BITS((pstSnsState->au32FL[0] + 8) >> 1);
    pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = LOW_8BITS((pstSnsState->au32FL[0] + 8) >> 1);
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    u32Val = (u32IntTime > SC2231_FULL_LINES_MAX) ? SC2231_FULL_LINES_MAX : u32IntTime;

    pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = (u32Val >> 12) & 0x0F;
    pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = (u32Val >> 4)  & 0xFF;
    pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = (u32Val << 4)  & 0xF0;

    return;
}

static const HI_U16 gau16GainIndex[128] = {
...
static const HI_U16 gau16AGainTable[128] = {
    0x0320, 0x0321, 0x0322, 0x0323, 0x0324, 0x0325, 0x0326, 0x0327, 0x0328, 0x0329, 0x032a, 0x032b, 0x032c, 0x032d, 0x032e, 0x032f, 0x0330, 0x0331, 0x0332, 0x0333, 0x0334, 0x0335, 0x0336, 0x0337, 0x0338, 0x0339, 0x033a, 0x033b, 0x033c, 0x033d, 0x033e, 0x033f,
    0x0720, 0x0721, 0x0722, 0x0723, 0x0724, 0x0725, 0x0726, 0x0727, 0x0728, 0x0729, 0x072a, 0x072b, 0x072c, 0x072d, 0x072e, 0x072f, 0x0730, 0x0731, 0x0732, 0x0733, 0x0734, 0x0735, 0x0736, 0x0737, 0x0738, 0x0739, 0x073a, 0x073b, 0x073c, 0x073d, 0x073e, 0x073f,
    0x0f20, 0x0f21, 0x0f22, 0x0f23, 0x0f24, 0x0f25, 0x0f26, 0x0f27, 0x0f28, 0x0f29, 0x0f2a, 0x0f2b, 0x0f2c, 0x0f2d, 0x0f2e, 0x0f2f, 0x0f30, 0x0f31, 0x0f32, 0x0f33, 0x0f34, 0x0f35, 0x0f36, 0x0f37, 0x0f38, 0x0f39, 0x0f3a, 0x0f3b, 0x0f3c, 0x0f3d, 0x0f3e, 0x0f3f,
...
static const HI_U16 gau16DGainTable[160] = {
    0x0080, 0x0084, 0x0088, 0x008C, 0x0090, 0x0094, 0x0098, 0x009C, 0x00A0, 0x00A4, 0x00A8, 0x00AC, 0x00B0, 0x00B4, 0x00B8, 0x00BC, 0x00C0, 0x00C4, 0x00C8, 0x00CC, 0x00D0, 0x00D4, 0x00D8, 0x00DC, 0x00E0, 0x00E4, 0x00E8, 0x00EC, 0x00F0, 0x00F4, 0x00F8, 0x00FC,
    0x0180, 0x0184, 0x0188, 0x018C, 0x0190, 0x0194, 0x0198, 0x019C, 0x01A0, 0x01A4, 0x01A8, 0x01AC, 0x01B0, 0x01B4, 0x01B8, 0x01BC, 0x01C0, 0x01C4, 0x01C8, 0x01CC, 0x01D0, 0x01D4, 0x01D8, 0x01DC, 0x01E0, 0x01E4, 0x01E8, 0x01EC, 0x01F0, 0x01F4, 0x01F8, 0x01FC,
    0x0380, 0x0384, 0x0388, 0x038C, 0x0390, 0x0394, 0x0398, 0x039C, 0x03A0, 0x03A4, 0x03A8, 0x03AC, 0x03B0, 0x03B4, 0x03B8, 0x03BC, 0x03C0, 0x03C4, 0x03C8, 0x03CC, 0x03D0, 0x03D4, 0x03D8, 0x03DC, 0x03E0, 0x03E4, 0x03E8, 0x03EC, 0x03F0, 0x03F4, 0x03F8, 0x03FC,
    0x0780, 0x0784, 0x0788, 0x078C, 0x0790, 0x0794, 0x0798, 0x079C, 0x07A0, 0x07A4, 0x07A8, 0x07AC, 0x07B0, 0x07B4, 0x07B8, 0x07BC, 0x07C0, 0x07C4, 0x07C8, 0x07CC, 0x07D0, 0x07D4, 0x07D8, 0x07DC, 0x07E0, 0x07E4, 0x07E8, 0x07EC, 0x07F0, 0x07F4, 0x07F8, 0x07FC,
    0x0F80, 0x0F84, 0x0F88, 0x0F8C, 0x0F90, 0x0F94, 0x0F98, 0x0F9C, 0x0FA0, 0x0FA4, 0x0FA8, 0x0FAC, 0x0FB0, 0x0FB4, 0x0FB8, 0x0FBC, 0x0FC0, 0x0FC4, 0x0FC8, 0x0FCC, 0x0FD0, 0x0FD4, 0x0FD8, 0x0FDC, 0x0FE0, 0x0FE4, 0x0FE8, 0x0FEC, 0x0FF0, 0x0FF4, 0x0FF8, 0x0FFC
};
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    } else if (u8Reg0x3e08 >= 0x07 && u8Reg0x3e08 < 0x0f) { /* 0x0710~0x071f 4x>gain >= 2x */
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data  =  0x00;
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data =  0x40;
        pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =  0x88;
        pstSnsState->astRegsInfo[0].astI2cData[12].u32Data =  0x30;
    } else if (u8Reg0x3e08 >= 0x0f && u8Reg0x3e08 < 0x1f) { /* 0x0f10~0x0f1f 8x>gain >= 4x */
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data  =  0x00;
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data =  0x40;
        pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =  0x88;
        pstSnsState->astRegsInfo[0].astI2cData[12].u32Data =  0x30;
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        sc2231_write_register(ViPipe, SC2231_VMAX_H_ADDR, (u32FullLines_5Fps & 0xFF00) >> 8);  /* 5fps */
        sc2231_write_register(ViPipe, SC2231_VMAX_L_ADDR, (u32FullLines_5Fps & 0xFF));         /* 5fps */
        sc2231_write_register(ViPipe, SC2231_EXP_M_ADDR, (u32MaxIntTime_5Fps & 0x0FF0) >> 4); /* max exposure lines */
        sc2231_write_register(ViPipe, SC2231_EXP_L_ADDR, (u32MaxIntTime_5Fps & 0xF) << 4);    /* max exposure lines */

        sc2231_write_register(ViPipe, SC2231_AGAIN_H_ADDR, 0x00);  /* AG, Context A */
        sc2231_write_register(ViPipe, SC2231_AGAIN_L_ADDR, 0x10);  /* DG, Context A */
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > SC2231_FULL_LINES_MAX * 2) ? (SC2231_FULL_LINES_MAX * 2) : pstSnsState->u32FLStd;
        sc2231_write_register(ViPipe, SC2231_VMAX_H_ADDR, (pstSnsState->u32FLStd & 0xFF00) >> 8);    /* 30fps */
        sc2231_write_register(ViPipe, SC2231_VMAX_L_ADDR, (pstSnsState->u32FLStd & 0xFF));
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunSc2231BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = sc2231_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = sc2231_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = sc2231_data_byte;
        }

        // Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = SC2231_EXP_H_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = SC2231_EXP_M_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = SC2231_EXP_L_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = SC2231_DGAIN_H_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = SC2231_DGAIN_L_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = SC2231_AGAIN_H_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = SC2231_AGAIN_L_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = SC2231_VMAX_H_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = SC2231_VMAX_L_ADDR;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (SC2231_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                u8SensorImageMode     = SC2231_SENSOR_1080P_30FPS_LINEAR_MODE;
...
static HI_S32 sc2231_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, SC2231_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC2231_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC2231_ID);
    if (s32Ret != HI_SUCCESS) {
