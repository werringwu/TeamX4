...
HI_S32 sc2231_GetQuickStartFlag(VI_PIPE ViPipe, HI_U32 *quickStartFlag)
{
    HI_S32 s32Ret = 0;
    ISP_MOD_PARAM_S stModParam;
    s32Ret = HI_MPI_ISP_GetModParam(&stModParam);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sc2231 Get QuickStart Error\n");
        return s32Ret;
    }
...
int sc2231_read_register(VI_PIPE ViPipe, HI_U32 addr)
{
    HI_S32 s32RegVal = 0;
    if (g_fd[ViPipe] < 0) {
        ISP_TRACE(HI_DBG_ERR, "sc2231_read_register fd not opened!\n");
        return HI_FAILURE;
    }

    HI_S32 s32Ret = 0;
    HI_U32 u32RegWidth = sc2231_addr_byte;
    HI_U32 u32DataWidth = sc2231_data_byte;
    HI_U8 aRecvbuf[4];



#ifdef LOSCFG_HOST_TYPE_HIBVT
    HI_U32 u32SnsI2cAddr = (sc2231_i2c_addr >> 1);
...
int sc2231_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
...
void sc2231_init(VI_PIPE ViPipe)
{
    ...
    if (quickStartFlag == 0)    {
        ...
        delay_ms(10);
        for (i = 0; i < g_pastSc2231[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
            sc2231_write_register(ViPipe, g_pastSc2231[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastSc2231[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
        }
    }
    g_pastSc2231[ViPipe]->bInit = HI_TRUE;
