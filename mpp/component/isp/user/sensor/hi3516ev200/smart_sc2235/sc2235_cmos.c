...
ISP_SNS_STATE_S *g_pastSc2235[ISP_MAX_PIPE_NUM] = {HI_NULL};
...
#define SC2235_SENSOR_RESET_CTX(dev)         (g_pastSc2235[dev] = HI_NULL)
...
#define SC2235_INCREASE_LINES                 (0) /* make real fps less than stand fps because NVR require */
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {
        case SC2235_SENSOR_1080P_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30) && (f32Fps > 8.229)) {
                u32VMAX = SC2235_VMAX_1080P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
    ...
    u32VMAX = (u32VMAX > SC2235_FULL_LINES_MAX) ? SC2235_FULL_LINES_MAX : u32VMAX;
    pstSnsState->u32FLStd = u32VMAX;

    pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = ((u32VMAX & 0xFF00) >> 8);
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    SC2235_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
    u32FullLines = (u32FullLines > SC2235_FULL_LINES_MAX) ? SC2235_FULL_LINES_MAX : u32FullLines;
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    u32Val = (u32IntTime > SC2235_FULL_LINES_MAX) ? SC2235_FULL_LINES_MAX : u32IntTime;
    pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = (u32Val >> 4) & 0xFF;
    pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = (u32Val << 4) & 0xF0;
...
static HI_VOID cmos_gains_update(VI_PIPE ViPipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    if (u16AgainTab[u32Again] < 2 * 1024) {
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data  = 0x05; // 0x3301
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = 0x84; // 0x3631
        pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = 0x2f; // 0x366f
    ...
    // Sensor_Internal_DPC
    if (gain_all < 10 * 1024) {
        pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = 0x04; // 0x5781
        pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = 0x18; // 0x5785
    } else {
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        sc2235_write_register(ViPipe, SC2235_VMAX0_ADDR, (u32FullLines_5Fps & 0xFF00) >> 8);  /* 5fps */
        sc2235_write_register(ViPipe, SC2235_VMAX1_ADDR, u32FullLines_5Fps & 0xFF);           /* 5fps */
        sc2235_write_register(ViPipe, SC2235_EXPTIME0_ADDR, ((u32MaxIntTime_5Fps >> 4) & 0xFF));   /* max exposure lines */
        sc2235_write_register(ViPipe, SC2235_EXPTIME1_ADDR, ((u32MaxIntTime_5Fps << 4) & 0xF0));   /* max exposure lines */
        sc2235_write_register(ViPipe, SC2235_GAIN0_ADDR, 0x00);                                    /* min Gain = 1 */
        sc2235_write_register(ViPipe, SC2235_GAIN1_ADDR, 0x03);
        sc2235_write_register(ViPipe, SC2235_GAIN2_ADDR, 0x10);
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > SC2235_FULL_LINES_MAX) ? SC2235_FULL_LINES_MAX : pstSnsState->u32FLStd;
        pstSnsState->au32FL[0] = pstSnsState->u32FLStd;
        sc2235_write_register(ViPipe, SC2235_VMAX0_ADDR, ((pstSnsState->u32FLStd & 0xFF00) >> 8));  /* Standard FPS */
        sc2235_write_register(ViPipe, SC2235_VMAX1_ADDR, (pstSnsState->u32FLStd & 0xFF));           /* Standard FPS */
    ...
    switch (u8Mode) {
        case WDR_MODE_NONE:
            ...
            if (pstSnsState->u8ImgMode == SC2235_SENSOR_1080P_30FPS_LINEAR_MODE) {
                ...
                ISP_TRACE(HI_DBG_ERR, "SC2235_SENSOR_1080P_30FPS_LINEAR_MODE\n");
            } else {
                ISP_TRACE(HI_DBG_ERR, "cmos_set_wdr_mode_Mode's name is wrong\n");
                return HI_FAILURE;
            }
            break;
        default:
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunSc2235BusInfo[ViPipe].s8I2cDev;

        if (g_astIspCtx[ViPipe].stBlockAttr.enIspRunningMode == ISP_MODE_RUNNING_ONLINE) {
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = sc2235_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = sc2235_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = sc2235_data_byte;
        }
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr  = SC2235_EXPTIME0_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr  = SC2235_EXPTIME1_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr  = SC2235_GAIN1_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr  = SC2235_GAIN2_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr  = SC2235_VMAX0_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr  = SC2235_VMAX1_ADDR;
        ...
        if (HI_TRUE == (pstSnsState->astRegsInfo[0].astI2cData[0].bUpdate ||  pstSnsState->astRegsInfo[0].astI2cData[1].bUpdate || pstSnsState->astRegsInfo[0].astI2cData[2].bUpdate ||  pstSnsState->astRegsInfo[0].astI2cData[3].bUpdate)) {
            pstSnsState->astRegsInfo[0].astI2cData[6].bUpdate  = HI_TRUE;    // Grouphold_AEC/AGC_Logic
            ...
            pstSnsState->astRegsInfo[0].astI2cData[6].bUpdate  = HI_FALSE;    // Grouphold_AEC/AGC_Logic
            pstSnsState->astRegsInfo[0].astI2cData[7].bUpdate  = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[8].bUpdate  = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[9].bUpdate  = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[10].bUpdate = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[11].bUpdate = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[12].bUpdate = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[13].bUpdate = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[14].bUpdate = HI_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[15].bUpdate = HI_FALSE;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if ((pstSensorImageMode->u16Width <= 1920) && (pstSensorImageMode->u16Height <= 1080)) {
        ...
        if (pstSensorImageMode->f32Fps <= 30) {
            ...
            ISP_TRACE(HI_DBG_ERR, "Not support! Width:%d, Height:%d, Fps:%f\n",
...
static HI_S32 sc2235_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, SC2235_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC2235_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC2235_ID);
    if (s32Ret != HI_SUCCESS) {
