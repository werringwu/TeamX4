...
int sc2235_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
...
void sc2235_init(VI_PIPE ViPipe)
{
    ...
    switch (u8ImgMode) {
        ...
        default:
            ...
            g_pastSc2235[ViPipe]->bInit = HI_FALSE;
            break;
    }

    for (i = 0; i < g_pastSc2235[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        sc2235_write_register(ViPipe, g_pastSc2235[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastSc2235[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }

    g_pastSc2235[ViPipe]->bInit = HI_TRUE;
