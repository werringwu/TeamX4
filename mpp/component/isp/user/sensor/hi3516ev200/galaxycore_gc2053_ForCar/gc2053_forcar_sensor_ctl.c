 ...
 ******************************************************************************
  File Name     : gc2053_forcar_sensor_ctl.c
...
const unsigned char gc2053_forcar_i2c_addr     =    0x6e;        /* I2C Address of GC2053_forcar */
...
int gc2053_forcar_i2c_init(VI_PIPE ViPipe)
{
    ...
    ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, (gc2053_forcar_i2c_addr >> 1));
    if (ret < 0) {
...
int gc2053_forcar_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
    ...
    ret = write(g_fd[ViPipe], buf, gc2053_forcar_addr_byte + gc2053_forcar_data_byte);
    if (ret < 0) {
...
void gc2053_forcar_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
...
void gc2053_forcar_init(VI_PIPE ViPipe)
{
    ...
    switch (u8ImgMode) {
        ...
        default:
            ...
            g_pastGc2053_forcar[ViPipe]->bInit = HI_FALSE;
            break;
    }


    for (i = 0; i < g_pastGc2053_forcar[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        gc2053_forcar_write_register(ViPipe, g_pastGc2053_forcar[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastGc2053_forcar[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }

    g_pastGc2053_forcar[ViPipe]->bInit = HI_TRUE;
