 ...
 ******************************************************************************
  File Name     : gc2053_forcar_cmos.c
...
ISP_SNS_STATE_S *g_pastGc2053_forcar[ISP_MAX_PIPE_NUM] = {HI_NULL};

#define GC2053_FORCAR_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastGc2053_forcar[dev])
#define GC2053_FORCAR_SENSOR_SET_CTX(dev, pstCtx)   (g_pastGc2053_forcar[dev] = pstCtx)
#define GC2053_FORCAR_SENSOR_RESET_CTX(dev)         (g_pastGc2053_forcar[dev] = HI_NULL)
...
extern void gc2053_forcar_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip);
    ...
    switch (pstSnsState->u8ImgMode) {
        case GC2053_SENSOR_1080P_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30) && (f32Fps > 2.06)) {
                u32VMAX = GC2053_VMAX_1080P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = 0x0;
    pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = (u32FullLines & 0xFF);
    pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = ((u32FullLines & 0xFF00) >> 8);
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    pstSnsState->astRegsInfo[0].astI2cData[1].u32Data = ((u32Val & 0xFF00) >> 8);
    return;
}
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunGc2053_forcar_BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = gc2053_forcar_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = gc2053_forcar_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = gc2053_forcar_data_byte;
        }
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum - 2; i++) {                              // AEC/AGC_Update
...
static HI_S32 gc2053_forcar_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, GC2053_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, GC2053_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, GC2053_ID);
    if (s32Ret != HI_SUCCESS) {
