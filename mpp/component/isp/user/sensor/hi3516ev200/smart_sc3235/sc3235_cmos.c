...
ISP_SNS_STATE_S *g_pastSc3235[ISP_MAX_PIPE_NUM] = {HI_NULL};
...
#define SC3235_SENSOR_RESET_CTX(dev)         (g_pastSc3235[dev] = HI_NULL)
...
{
    HI_U32 u32FullLines;
    HI_U32 u32VMAX = SC3235_VMAX_1296P30_LINEAR;
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        if (HI_TRUE == (pstSnsState->astRegsInfo[0].astI2cData[3].bUpdate\
...
static HI_S32 sc3235_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(ViPipe, SC3235_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC3235_ID);
    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC3235_ID);
    if (s32Ret != HI_SUCCESS) {
