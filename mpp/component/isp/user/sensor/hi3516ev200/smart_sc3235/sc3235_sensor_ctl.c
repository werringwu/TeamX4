...
int sc3235_read_register(VI_PIPE ViPipe, HI_U32  addr)
{
    HI_S32 s32RegVal = 0;
    if (g_fd[ViPipe] < 0) {
        ISP_TRACE(HI_DBG_ERR, "sc3235_read_register fd not opened!\n");
        return HI_FAILURE;
    }

    HI_S32 s32Ret = 0;
    HI_U32 u32RegWidth = sc3235_addr_byte;
    HI_U32 u32DataWidth = sc3235_data_byte;
    HI_U8 aRecvbuf[4];



#ifdef LOSCFG_HOST_TYPE_HIBVT
    HI_U32 u32SnsI2cAddr = (sc3235_i2c_addr >> 1);
...
int sc3235_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
...
void sc3235_init(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < g_pastSc3235[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        sc3235_write_register(ViPipe, g_pastSc3235[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastSc3235[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }

    g_pastSc3235[ViPipe]->bInit = HI_TRUE;
...
void sc3235_linear_1296p30_init(VI_PIPE ViPipe)
{
    ...
    sc3235_write_register (ViPipe, 0x391d, 0x04);
