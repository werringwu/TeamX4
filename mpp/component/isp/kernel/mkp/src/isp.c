...
HI_S32 ISP_DRV_FE_StatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    ...
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
    if (pstDrvCtx->stWDRAttr.bMastPipe == HI_FALSE) {
        return HI_SUCCESS;
    }
...
HI_S32 ISP_DRV_BeOfflineStitchStatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    ...
    if ((pstDrvCtx->stStitchAttr.bMainPipe != HI_TRUE) && (pstDrvCtx->stStitchAttr.bStitchEnable != HI_TRUE)) {
        return  HI_SUCCESS;
    }
    ...
    for (k = 0; k < pstDrvCtx->stStitchAttr.u8StitchPipeNum; k++) {
        ...
        if (stBeStitch.pstSyncBeStt[k] == HI_NULL) {
...
HI_S32 ISP_DRV_StatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable == HI_TRUE) {
        if (pstDrvCtx->stStitchAttr.bMainPipe == HI_TRUE) {
...
HI_S32 ISP_DRV_SetLdciSttAddr(VI_PIPE ViPipe, HI_U64 u64ReadSttAddr, HI_U64 u64WriteSttAddr)
{
    ...
    HI_S32 s32Ret;
    HI_U32 u32Size = sizeof(ISP_LDCI_STAT_S);
    HI_U64 u64WriteAddr;
    ISP_BE_REGS_ATTR_S  stIspBeRegsAttr = { 0 };
...
HI_S32 ISP_DRV_UpdateLdciTprOnlineAttr(VI_PIPE ViPipe, ISP_STAT_S *pstStat)
{
    HI_U8 u8CurReadBufIdx;
    ISP_DRV_CTX_S   *pstDrvCtx   = HI_NULL;
    ISP_LDCI_STAT_S *pstLdciStat = HI_NULL;
...
HI_S32 ISP_DRV_UpdateLdciNormalOfflineAttr(VI_PIPE ViPipe, ISP_BE_SYNC_PARA_S *pstBeSyncPara)
{
...
HI_S32 ISP_DRV_UpdateLdciTprOfflineAttr(VI_PIPE ViPipe, ISP_BE_SYNC_PARA_S *pstBeSyncPara)
{
    HI_U8 i;
    ISP_DRV_CTX_S *pstDrvCtx = HI_NULL;
...
HI_S32 ISP_DRV_LdciOnlineAttrUpdate(VI_PIPE ViPipe, ISP_STAT_S *pstStat)
{
    ISP_CHECK_PIPE(ViPipe);
...
static HI_VOID ISP_DRV_LdciBufAttrReset(ISP_DRV_CTX_S *pstDrvCtx)
{
    ...
    pstDrvCtx->stLdciReadBufAttr.bInit  = HI_FALSE;
    pstDrvCtx->stLdciWriteBufAttr.bInit = HI_FALSE;
    ...
    for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
        ...
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].pVirAddr   = HI_NULL;
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].u64Size    = 0;
    }

    for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].u64PhyAddr = 0;
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].pVirAddr   = HI_NULL;
...
HI_S32 ISP_DRV_LdciTprBufInit(VI_PIPE ViPipe)
{
    ...
    s32Ret = CMPI_MmzMallocNocache(HI_NULL, acName, &u64PhyAddr, (HI_VOID **)&pu8VirAddr, u64Size * u8BufNumAll);

    if (s32Ret != HI_SUCCESS) {
        ISP_TRACE(HI_DBG_ERR, "ISP[%d] MmzMalloc Ldci buffer Failure!\n", ViPipe);
        return HI_ERR_ISP_NOMEM;
    }
    ...
    for (i = 0; i < u8WriteBufNum; i++) {
        ...
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].pVirAddr   = (HI_VOID *)(pu8VirAddr + i * u64Size);
    ...
    for (i = 0; i < u8ReadBufNum; i++) {
        ...
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].pVirAddr   = (HI_VOID *)(pu8VirAddr + j * u64Size);
    ...
    if (IS_ONLINE_MODE(pstDrvCtx->stWorkMode.enIspRunningMode) ||
        ...
        if (s32Ret != HI_SUCCESS) {
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] Set Ldci Param/OutParam addr Err!\n", ViPipe);
            goto fail0;
        }
...
HI_S32 ISP_DRV_LdciNormalBufInit(VI_PIPE ViPipe)
{
    ...
    u64PhyAddr = CMPI_MmzMalloc(HI_NULL, acName, u64Size * u8BufNum);
    if (u64PhyAddr == 0) {
        ISP_TRACE(HI_DBG_ERR, "alloc ISP[%d] Ldci buf err\n", ViPipe);
        return HI_ERR_ISP_NOMEM;
    }
    ...
    if (IS_SIDEBYSIDE_MODE(pstDrvCtx->stWorkMode.enIspRunningMode) ||
        ...
        if (s32Ret != HI_SUCCESS) {
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] Set Ldci stt addr Err!\n", ViPipe);
            goto fail0;
        }
...
fail0:
    ...
    if (u64PhyAddr != 0) {
        CMPI_MmzFree(u64PhyAddr, HI_NULL);
    }
...
HI_S32 ISP_DRV_LdciReadSttBufGet(VI_PIPE ViPipe, ISP_LDCI_READ_STT_BUF_S *pstLdciReadSttBuf)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = HI_NULL;
...
HI_S32 ISP_DRV_SwitchMode(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{
    ...
    if (pstCfgNode != HI_NULL) {
        ...
        ISP_DRV_RegConfigWdr(ViPipe, apstBeReg, pstDrvCtx, &pstCfgNode->stWDRRegCfg, u8BlkNum, u8BlkDev,
                             pstDrvCtx->stWDRCfg.au32ExpRatio);
...
static HI_S32 ISP_DRV_CalcExpRatio(ISP_SYNC_CFG_S *pstSyncCfg, ISP_SYNC_CFG_BUF_NODE_S *pstCurNode,
                                   ISP_SYNC_CFG_BUF_NODE_S *pstPreNode, HI_U64 *au64CurSnsGain,
...
static HI_S32 ISP_DRV_CalcDrcComp(ISP_SYNC_CFG_S *pstSyncCfg, ISP_SYNC_CFG_BUF_NODE_S *pstCurNode,
...
HI_S32 ISP_DRV_CalcSyncCfg(ISP_SYNC_CFG_S *pstSyncCfg)
{
    ...
    if (pstSyncCfg->apstNode[1] == HI_NULL) {
        ...
        if (pstSyncCfg->apstNode[1]->bValid == HI_FALSE) {
            pstPreNode = pstSyncCfg->apstNode[0];
        } else {
...
HI_S32 ISP_DRV_RegConfigIsp(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{
    ...
    if (pstDrvCtx->stSnapInfoLoad.enSnapState == SNAP_STATE_CFG) {
        ...
        if (pstDrvCtx->stSnapAttr.bLoadCCM == HI_TRUE) {
        ...
        if (pstDrvCtx->stStitchAttr.bStitchEnable == HI_TRUE) {
            if (pstDrvCtx->stStitchAttr.bMainPipe == HI_FALSE) {
                MainPipeS  = pstDrvCtx->stStitchAttr.as8StitchBindId[0];
...
HI_S32 ISP_DRV_StitchRegsCfgSensor(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx, HI_U32 u32UpdatePos)
{
        ...
        for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
            ...
            if (pstCfgNode->bValid == HI_TRUE) {
                ...
                if ((pstSspData->bUpdate == HI_TRUE) && (u32UpdatePos == pstSspData->u8IntPos)) {
                    if (pstDrvCtx->stBusCb.pfnISPWriteSSPData != HI_NULL) {
    ...
    if ((pstCfgNode->bValid == HI_TRUE) && (pstCfgNode->stSnsRegsInfo.stSlvSync.bUpdate == HI_TRUE)) {
...
HI_S32 ISP_SetProNrParam(VI_PIPE ViPipe, const ISP_PRO_BNR_PARAM_S *pstProNrParam)
{
    ...
    HI_U32 u32ParamNum;
    ISP_DRV_CTX_S *pstDrvCtx = HI_NULL;
    ISP_PRO_NR_PARAM_S *pstProNrParamCtx = HI_NULL;
    ISP_NR_AUTO_ATTR_S *pstProNrAttr = HI_NULL;
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstProNrParam);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);

    if (pstProNrParam->pastNrAttr == HI_NULL) {
        ISP_TRACE(HI_DBG_ERR, "NrParam buf address can't null!\n");
        s32Ret =  HI_ERR_ISP_NOT_INIT;
        goto failed0;
    }

    s32Ret = read_user_linear_space_valid((HI_U8 *)(pstProNrParam->pastNrAttr), sizeof(ISP_NR_AUTO_ATTR_S));
    if (s32Ret == HI_FALSE) {
    ...
    if ((u32ParamNum > PRO_MAX_FRAME_NUM) || (u32ParamNum < 1)) {
        ISP_TRACE(HI_DBG_ERR, "Invalid u32ParamNum value %d! Value range: [1, %d]\n", u32ParamNum, PRO_MAX_FRAME_NUM);
    ...
    if (pstDrvCtx->stProInfo.bInit == HI_FALSE) {
        ISP_TRACE(HI_DBG_WARN, "ProInfo buf doesn't initialized!\n");
        s32Ret = HI_ERR_ISP_NOT_INIT;
        goto failed1;
    }

    if (pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr == HI_NULL) {
...
HI_S32 ISP_GetProNrParam(VI_PIPE ViPipe, ISP_PRO_BNR_PARAM_S *pstProNrParam)
{
    ...
    s32Ret = write_user_linear_space_valid((HI_U8 *)(pstProNrParam->pastNrAttr), sizeof(ISP_NR_AUTO_ATTR_S));
    if (s32Ret == HI_FALSE) {
    ...
    for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++) {
        for (j = 0; j < ISP_SHARPEN_GAIN_NUM; j++) {
            if (pstShpAttr->au16TextureStr[j][i] > 4095) {
                ISP_TRACE(HI_DBG_ERR, "Invalid au16TextureStr[%d][%d]:%d! Range:[0, 4095]\n",
                          j, i, pstShpAttr->au16TextureStr[j][i]);
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
            if (pstShpAttr->au16EdgeStr[j][i] > 4095) {
                ISP_TRACE(HI_DBG_ERR, "Invalid au16EdgeStr[%d][%d]:%d! Range:[0, 4095]\n",
                          j, i, pstShpAttr->au16EdgeStr[j][i]);
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
            if (pstShpAttr->au8LumaWgt[j][i] > 127) {
                ISP_TRACE(HI_DBG_ERR, "Invalid au16EdgeStr[%d][%d]:%d! Range:[0, 127]\n",
                          j, i, pstShpAttr->au8LumaWgt[j][i]);
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
        }
        if (pstShpAttr->au16TextureFreq[i] > 4095) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au16TextureFreq[%d]:%d! Range:[0, 4095]\n",
                      i, pstShpAttr->au16TextureFreq[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
        if (pstShpAttr->au16EdgeFreq[i] > 4095) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au16EdgeFreq[%d]:%d! Range:[0, 4095]\n", i, pstShpAttr->au16EdgeFreq[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
        if (pstShpAttr->au8OverShoot[i] > 127) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8OverShoot[%d]:%d! Range:[0, 127]\n", i, pstShpAttr->au8OverShoot[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
        if (pstShpAttr->au8UnderShoot[i] > 127) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8UnderShoot[%d]:%d! Range:[0, 127]\n", i, pstShpAttr->au8UnderShoot[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (pstShpAttr->au8EdgeFiltStr[i] > 63) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8EdgeFiltStr[%d]:%d! Range:[0, 63]\n", i, pstShpAttr->au8EdgeFiltStr[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (pstShpAttr->au8RGain[i] > HI_ISP_SHARPEN_RGAIN) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8RGain[%d]:%d! Range:[0, %d]\n",
                      i, pstShpAttr->au8RGain[i], HI_ISP_SHARPEN_RGAIN);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (pstShpAttr->au8BGain[i] > HI_ISP_SHARPEN_BGAIN) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8BGain[%d]:%d! Range:[0, %d]\n",
                      i, pstShpAttr->au8BGain[i], HI_ISP_SHARPEN_BGAIN);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }

        if (pstShpAttr->au8SkinGain[i] > 31) {
            ISP_TRACE(HI_DBG_ERR, "Invalid au8SkinGain[%d]:%d! Range:[0, 31]\n", i, pstShpAttr->au8SkinGain[i]);
            return HI_ERR_ISP_ILLEGAL_PARAM;
        }
    }
...
HI_S32 ISP_SetProShpParam(VI_PIPE ViPipe, const ISP_PRO_SHARPEN_PARAM_S *pstProShpParam)
{
    ...
    HI_U32 u32ParamNum;
    ISP_DRV_CTX_S *pstDrvCtx = HI_NULL;
    ISP_PRO_SHP_PARAM_S *pstProShpParamCtx = HI_NULL;
    ...
    if (pstProShpParam->pastShpAttr == HI_NULL) {
        ISP_TRACE(HI_DBG_ERR, "ShpParam buf address can't null!\n");
        s32Ret = HI_ERR_ISP_NULL_PTR;
        goto failed0;
    }

    s32Ret = read_user_linear_space_valid((HI_U8 *)(pstProShpParam->pastShpAttr), sizeof(ISP_SHARPEN_AUTO_ATTR_S));
    if (s32Ret == HI_FALSE) {
        ISP_TRACE(HI_DBG_ERR, "Pipe:%d pastShpAttr address is invalid!\n", ViPipe);
        s32Ret = HI_ERR_ISP_INVALID_ADDR;
        goto failed0;
    ...
    if (pstDrvCtx->stProInfo.stProShpParamInfo.pVirAddr == HI_NULL) {
        ISP_TRACE(HI_DBG_WARN, "ShpParam buf don't init ok!\n");
        s32Ret = HI_ERR_ISP_NOT_INIT;
        goto failed1;
    }
...
HI_S32 ISP_GetProShpParam(VI_PIPE ViPipe, ISP_PRO_SHARPEN_PARAM_S *pstProShpParam)
{
    ...
    s32Ret = write_user_linear_space_valid((HI_U8 *)(pstProShpParam->pastShpAttr), sizeof(ISP_SHARPEN_AUTO_ATTR_S));
    if (s32Ret == HI_FALSE) {
        ISP_TRACE(HI_DBG_ERR, "Pipe:%d pastShpAttr address is invalid!\n", ViPipe);
        return HI_ERR_ISP_INVALID_ADDR;
    }
...
HI_S32 ISP_DRV_ProcPrintf(VI_PIPE ViPipe, osal_proc_entry_t *s)
{
    ...
    if (pstDrvCtx->stPorcMem.pProcVirtAddr != HI_NULL) {
...
HI_S32 ISP_UpdateInfoSync(VI_PIPE ViPipe, ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo)
{
    ...
    if (pstDrvCtx->stTransInfo.stUpdateInfo.pVirAddr == HI_NULL) {
...
HI_S32 ISP_frameInfoSync(VI_PIPE ViPipe, ISP_FRAME_INFO_S *pstIspframeInfo)
{
    ...
    if (pstDrvCtx->stTransInfo.stFrameInfo.pVirAddr == HI_NULL) {
        ISP_TRACE(HI_DBG_WARN, "UpdateInfo buf don't init ok!\n");
        osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
        return HI_ERR_ISP_NOT_INIT;
    }
...
HI_S32 ISP_GetFrameInfo(VI_PIPE ViPipe, ISP_FRAME_INFO_S *pstIspFrame)
{
    ...
    if ((pstDrvCtx->bMemInit == HI_FALSE) && (pstDrvCtx->stWDRAttr.enWDRMode == WDR_MODE_NONE)) {
...
HI_S32 ISP_SetCtrlParam(VI_PIPE ViPipe, ISP_CTRL_PARAM_S *pstIspCtrlParam)
{
    ...
    if ((g_ProcParam[ViPipe] != 0) && (pstIspCtrlParam->u32ProcParam == 0) && (pstDrvCtx->bMemInit == HI_TRUE)) {
    ...
    if (g_LdciTprFltEn[ViPipe] != pstIspCtrlParam->bLdciTprFltEn) {
        if (pstDrvCtx->bMemInit == HI_TRUE) {
            ISP_TRACE(HI_DBG_ERR, "Vipipe:%d Does not support changed after isp init (bLdciTmpFltEn)!\n", ViPipe);
            return HI_ERR_ISP_NOT_SUPPORT;
        }
    }
...
HI_VOID ISP_DRV_StitchBeBufCtl(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < pstDrvCtx->stStitchAttr.u8StitchPipeNum; i++) {
        ...
        if (pstDrvCtxS->stBeBufInfo.bInit != HI_TRUE) {
...
HI_S32 ISP_DRV_WriteAllLdciSttAddr(VI_PIPE ViPipe)
{
    ...
    ISP_BE_WO_REG_CFG_S *pstBeRegCfg = HI_NULL;
    ISP_BE_BUF_NODE_S   *pstNode     = HI_NULL;
    ISP_DRV_CTX_S       *pstDrvCtx   = HI_NULL;
    ...
    for (k = 0; k < u8FreeNum; k++) {
        pstNode = ISP_QueueGetFreeBeBuf(&pstDrvCtx->stBeBufQueue);
        if (pstNode == HI_NULL) {
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] Get QueueGetFreeBeBuf fail!\r\n", ViPipe);
            return HI_FAILURE;;
        }
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable == HI_TRUE) {
        ...
        for (i = 0; i < pstDrvCtx->stStitchAttr.u8StitchPipeNum; i++) {
            ...
            if (pstDrvCtxS->enIspRunningState != ISP_BE_BUF_STATE_SWITCH) {
                osal_spin_unlock_irqrestore(pIspSpinLock, &u32Flags);
                ISP_TRACE(HI_DBG_WARN, "Pipe[%d] isp isn't  finish state!\n", ViPipe);
                return HI_FAILURE;
            }
        }
...
HI_S32 ISP_DRV_BeBufCtl(VI_PIPE ViPipe)
{
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable != HI_TRUE) {
        if (pstDrvCtx->stBeBufInfo.bInit != HI_TRUE) {
            ISP_TRACE(HI_DBG_ERR, "Pipe[%d] BeBuf (bInit != TRUE) !\n", ViPipe);
            return HI_FAILURE;
        }
...
static long ISP_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    switch (cmd) {
        ...
        case ISP_CLUT_BUF_GET: {
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);
            if (g_astIspDrvCtx[ViPipe].stClutBufAttr.bInit == HI_FALSE) {
                return HI_ERR_ISP_NOMEM;
            }
        ...
        case ISP_LDCI_READ_STT_BUF_GET: {
            return ISP_DRV_LdciReadSttBufGet(ViPipe, (ISP_LDCI_READ_STT_BUF_S *)(HI_UINTPTR_T)arg);
        }
        ...
        case ISP_STT_ADDR_INIT: {
            ...
            if (pstDrvCtx->stStitchAttr.bStitchEnable == HI_FALSE) {
        ...
        case ISP_BE_LUT_STT_BUF_GET: {
            ...
            if (g_astIspDrvCtx[ViPipe].stBeLut2SttAttr.bInit == HI_FALSE) {
                return HI_ERR_ISP_NOMEM;
            }
        ...
        case ISP_SPECAWB_BUF_GET: {
            ...
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);
            if (g_astIspDrvCtx[ViPipe].stSpecAwbBufAttr.bInit == HI_FALSE) {
                return HI_ERR_ISP_NOMEM;
            }
