...
static IntVGSCallBack s_pTdeCallBack;
static IntVGSCallBack s_pTdeRotateCallBack;
...
HI_S32 HIFB_DRV_Fill(HIFB_BUFFER_S *pDstImg, HI_U32 u32FillData)
{
    ...
    if (pDstImg == NULL) {
        HI_TRACE_FB(HI_DBG_ERR, "invalid parameter is NULL!\n");
        return HI_FAILURE;
    }
...
HI_S32 HIFB_DRV_Blit(HIFB_BUFFER_S *pSrcImg, HIFB_BUFFER_S *pDstImg, HIFB_BLIT_OPT_S *pstOpt, HI_BOOL bRefreshScreen)
{
    ...
    if ((pSrcImg == NULL) || (pDstImg == NULL) || (pstOpt == NULL)) {
        return HI_FAILURE;
    }
    ...
    if (((pSrcImg->stCanvas.u32Width == 0) || (pSrcImg->stCanvas.u32Height == 0))) {
        HI_TRACE_FB(HI_DBG_ERR, "invalid parameter!\n");
        return HI_FAILURE;
    }
    /** config src*/
    ...
    stDstSur.bYCbCrClut = HI_FALSE;
    stDstSur.enColorFmt = HIFB_DRV_ConvFmt(pDstImg->stCanvas.enFmt);
    stDstSur.u8Alpha0 = 0x0;
    stDstSur.u8Alpha1 = 0xff;
    ...
    if (pstOpt->bCallBack)
    {
        ...
        osal_memset(pParam, 0, sizeof(HIFB_TDECALLBACK_PARAM));
...
HI_S32 HIFB_DRV_Rotate(HIFB_BUFFER_S *pSrcImg, HIFB_BUFFER_S *pDstImg, HIFB_ROTATE_OPT_S *pstRotOpt, HI_BOOL bRefreshScreen)
{
    ...
    if ((pSrcImg == NULL) || (pDstImg == NULL) || (pstRotOpt == NULL)) {
        return HI_FAILURE;
    }
