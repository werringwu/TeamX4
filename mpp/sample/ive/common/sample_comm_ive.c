...
HI_S32 SAMPLE_COMM_IVE_VbInit(PIC_SIZE_E *paenSize,SIZE_S *pastSize,HI_U32 u32VpssChnNum)
{
    ...
    HI_U32 au32VpssBlk[VPSS_CHN_NUM] = {5,2};
    ...
    for (i = 0; i < u32VpssChnNum; i++)
    {
        ...
        stVbConf.astCommPool[i].u32BlkCnt  = au32VpssBlk[i];
    }
...
* function : Start Vo
******************************************************************************/
HI_S32 SAMPLE_COMM_IVE_StartVo(PIC_SIZE_E  enPicSize)
{
	HI_S32 s32Ret;
	...
	"SAMPLE_COMM_VO_StartVO failed,Error(%#x)!\n",s32Ret);
...
* function : Stop Vo
******************************************************************************/
HI_VOID SAMPLE_COMM_IVE_StopVo(PIC_SIZE_E  enPicSize)
{
	SAMPLE_VO_CONFIG_S stVoConfig;
	DYNAMIC_RANGE_E    enDynamicRange = DYNAMIC_RANGE_SDR8;
    stVoConfig.enDstDynamicRange = enDynamicRange;
    ...
    SAMPLE_IVE_SWITCH_S *pstSwitch,PIC_SIZE_E *penExtPicSize)
{
    ...
    ******************************************/
    ...
    {
        s32Ret = SAMPLE_COMM_IVE_StartVo(aenSize[0]);
        SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_INIT_4,
...
END_INIT_5:
    if (HI_TRUE == pstSwitch->bVo)
    {
        SAMPLE_COMM_IVE_StopVo(aenSize[0]);
    }
END_INIT_4:
...
HI_VOID SAMPLE_COMM_IVE_StopViVpssVencVo(SAMPLE_VI_CONFIG_S *pstViConfig,SAMPLE_IVE_SWITCH_S *pstSwitch)
{
    ...
    if (HI_TRUE == pstSwitch->bVo)
    {
		 (HI_VOID)SAMPLE_COMM_VI_GetSizeBySensor(pstViConfig->astViInfo[s32WorkSnsId].stSnsInfo.enSnsType, &enSize);
          SAMPLE_COMM_IVE_StopVo(enSize);
    }
