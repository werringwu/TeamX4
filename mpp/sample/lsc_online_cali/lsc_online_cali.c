...
HI_S32 SAMPLE_LSC_CALI_START_PREV(LSC_CALI_PREV_S *pstLsc_CaliPrev)
{
    ...
    pstLsc_CaliPrev->VencChn[0] = 0;
    pstLsc_CaliPrev->VpssGrp    = 0;
    pstLsc_CaliPrev->VpssChn    = VPSS_CHN1;
    pstLsc_CaliPrev->VoChn      = 0;

    for(i=0;i<VPSS_MAX_PHY_CHN_NUM;i++)
    {
        pstLsc_CaliPrev->abChnEnable[i] = 0;
    }

    /*config vi*/
    SAMPLE_COMM_VI_GetSensorInfo(&pstLsc_CaliPrev->stViConfig);

    pstLsc_CaliPrev->stViConfig.s32WorkingViNum                                   = s32ViCnt;
    pstLsc_CaliPrev->stViConfig.as32WorkingViId[0]                                = 0;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.MipiDev         = ViDev;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32BusId        = 0;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stDevInfo.ViDev           = ViDev;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stDevInfo.enWDRMode       = enWDRMode;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.enMastPipeMode = enMastPipeMode;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[0]       = ViPipe;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[1]       = -1;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stChnInfo.ViChn           = ViChn;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enPixFormat     = enPixFormat;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enDynamicRange  = enDynamicRange;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enVideoFormat   = enVideoFormat;
    pstLsc_CaliPrev->stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enCompressMode  = enCompressMode;
    ...
    /*start vi*/
    s32Ret = SAMPLE_COMM_VI_StartVi(&pstLsc_CaliPrev->stViConfig);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    astVpssChnAttr[VPSS_CHN1].u32Width                    = 1920;
    astVpssChnAttr[VPSS_CHN1].u32Height                   = 1080;
    ...
    /*start vpss*/
    pstLsc_CaliPrev->abChnEnable[VPSS_CHN1] = HI_TRUE;

    s32Ret = SAMPLE_COMM_VPSS_Start(pstLsc_CaliPrev->VpssGrp, pstLsc_CaliPrev->abChnEnable, &stVpssGrpAttr, astVpssChnAttr);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    /*vi bind vpss*/
    s32Ret = SAMPLE_COMM_VI_Bind_VPSS(ViPipe, ViChn, pstLsc_CaliPrev->VpssGrp);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    pstLsc_CaliPrev->stVoConfig.enDstDynamicRange = enDynamicRange;
    pstLsc_CaliPrev->stVoConfig.enVoIntfType = VO_INTF_BT1120;
    pstLsc_CaliPrev->stVoConfig.enIntfSync   = VO_OUTPUT_576P50;
    pstLsc_CaliPrev->stVoConfig.u32DisBufLen = 3;
    pstLsc_CaliPrev->stVoConfig.enPicSize = enPicSize;

    /*start vo*/
    s32Ret = SAMPLE_COMM_VO_StartVO(&pstLsc_CaliPrev->stVoConfig);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    /*vpss bind vo*/
    s32Ret = SAMPLE_COMM_VPSS_Bind_VO(pstLsc_CaliPrev->VpssGrp, VPSS_CHN1, pstLsc_CaliPrev->stVoConfig.VoDev, pstLsc_CaliPrev->VoChn);
    if (HI_SUCCESS != s32Ret)
    {
...
EXIT3:
    SAMPLE_COMM_VO_StopVO(&pstLsc_CaliPrev->stVoConfig);
EXIT2:
    SAMPLE_COMM_VPSS_Stop(pstLsc_CaliPrev->VpssGrp, pstLsc_CaliPrev->abChnEnable);
EXIT1:
    SAMPLE_COMM_VI_StopVi(&pstLsc_CaliPrev->stViConfig);
EXIT:
...
void usage(void)
{
    printf(
        "\n"
        "*************************************************\n"
        "Usage: ./lsc_online_cali [Pipe] [scale] \n"
        "ViDev: \n"
        "   0:Pipe_0 ~~ 5:Pipe_5\n"
        "scale: \n"
...
//  VIDEO_FRAME_INFO_S     stFrame;
    ...
    for(i=0;i<17;i++)
    {
        for(j = 0;j<17;j++)
        {
            fprintf(pFile,"%d,",stIspMLSCTable.stLscGainLut.au16RGain[i*17+j]);
        }
        fprintf(pFile,"\n");
    }
    fprintf(pFile,"Gr\n");
    for(i=0;i<17;i++)
    {
        for(j = 0;j<17;j++)
        {
            fprintf(pFile,"%d,",stIspMLSCTable.stLscGainLut.au16GrGain[i*17+j]);
        }
        fprintf(pFile,"\n");
    }
    fprintf(pFile,"Gb\n");
    for(i=0;i<17;i++)
    {
        for(j = 0;j<17;j++)
        {
            fprintf(pFile,"%d,",stIspMLSCTable.stLscGainLut.au16GbGain[i*17+j]);
        }
        fprintf(pFile,"\n");
    }
    fprintf(pFile,"B\n");
    for(i=0;i<17;i++)
    {
        for(j = 0;j<17;j++)
        {
            fprintf(pFile,"%d,",stIspMLSCTable.stLscGainLut.au16BGain[i*17+j]);
        }
        fprintf(pFile,"\n");
    }
