...
*/
#ifndef __HI_SCENECOMM_H__
#define __HI_SCENECOMM_H__
...
#define HI_SCENECOMM_CHECK_RETURN(ret, errcode) \
    do { \
        if (HI_SUCCESS != ret){   \
            SCLOGE(RED" ret[%08x]"NONE"\n", ret); \
            return errcode;  \
        }   \
    }while(0)
/** Return Result UnReturn */
#define HI_SCENECOMM_CHECK_UNRETURN(ret) \
    do { \
        if (HI_SUCCESS != ret){   \
            SCLOGE(RED" ret[%08x]"NONE"\n", ret); \
        }   \
...
#define HI_SCENECOMM_CHECK(ret, errcode) \
    do { \
        if (HI_SUCCESS != ret){   \
            SCLOGE(RED" ret[%08x] errcode[%x]"NONE"\n", ret, errcode); \
        }   \
    }while(0)
