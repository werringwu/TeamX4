...
static HI_S32 Setconfiginfo(HI_HANDLE chnHandle,
                        HI_BOOL isKeyByCA,
                        ...
                        const HI_U8 u8IVBuf[16])
{
    ...
    {
        CipherCtrl.stChangeFlags.bit1IV = CIPHER_IV_CHANGE_ALL_PKG;  //must set for CBC , CFB mode
...
#ifndef CHIP_TYPE_hi3516ev200
static HI_S32 multiCipher_DES_ECB_ENC(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    for(i = 0; i < 3; i++)
    {
        cipherDataArray[i].szSrcPhyAddr = POINT_TO_U32(HI_MMZ_New(u32TestDataLen, 0, NULL, "CIPHER_BufIn"));
        if (0 == cipherDataArray[i].szSrcPhyAddr)
        {
            HI_ERR_CIPHER("Error: Get phyaddr for input failed!\n");
            s32Ret = HI_FAILURE;
            goto __CIPHER_EXIT__;
        }
        pInputAddrVir[i] = HI_MMZ_Map(cipherDataArray[i].szSrcPhyAddr, u32TestDataLen, u32Testcached);

        cipherDataArray[i].szDestPhyAddr = POINT_TO_U32(HI_MMZ_New(u32TestDataLen, 0, NULL, "CIPHER_BufOut"));
        if (0 == cipherDataArray[i].szDestPhyAddr)
        {
            HI_ERR_CIPHER("Error: Get phyaddr for outPut failed!\n");
            s32Ret = HI_FAILURE;
            goto __CIPHER_EXIT__;
        }
        pOutputAddrVir[i] = HI_MMZ_Map(cipherDataArray[i].szDestPhyAddr, u32TestDataLen, u32Testcached);
...
__CIPHER_EXIT__:
    ...
    for(i = 0; i < 3; i++)
    {
        HI_MMZ_Unmap(cipherDataArray[i].szSrcPhyAddr);
        HI_MMZ_Delete(cipherDataArray[i].szSrcPhyAddr);

        HI_MMZ_Unmap(cipherDataArray[i].szDestPhyAddr);
        HI_MMZ_Delete(cipherDataArray[i].szDestPhyAddr);
    }
...
static HI_S32 multiCipher_DES_ECB_DEC(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 8;
    HI_U32 u32Testcached = 0;
    ...
    for(i = 0; i < 3; i++)
    {
        cipherDataArray[i].szSrcPhyAddr = POINT_TO_U32(HI_MMZ_New(u32TestDataLen, 0, NULL, "cipherIn"));
        if (0 == cipherDataArray[i].szSrcPhyAddr)
        {
            HI_ERR_CIPHER("Error: Get phyaddr for input failed!\n");
            s32Ret = HI_FAILURE;
        ...
        cipherDataArray[i].szDestPhyAddr = POINT_TO_U32(HI_MMZ_New(u32TestDataLen, 0, NULL, "cipherOut"));
        if (0 == cipherDataArray[i].szDestPhyAddr)
        {
            HI_ERR_CIPHER("Error: Get phyaddr for outPut failed!\n");
            s32Ret = HI_FAILURE;
...
static HI_S32 multiCipher_ex_AES_CBC_ENC(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        {
            HI_ERR_CIPHER("MultiCipher AES CBC Encryption run failed on array %d!\n", i);
...
__CIPHER_EXIT__:
    ...
    for(i = 0; i < 3; i++)
    {
        if(cipherDataArray[i].szSrcPhyAddr > 0)
        {
            HI_MMZ_Unmap(cipherDataArray[i].szSrcPhyAddr);
            HI_MMZ_Delete(cipherDataArray[i].szSrcPhyAddr);
        }
        if(cipherDataArray[i].szDestPhyAddr > 0)
        {
            HI_MMZ_Unmap(cipherDataArray[i].szDestPhyAddr);
            HI_MMZ_Delete(cipherDataArray[i].szDestPhyAddr);
        }
...
static HI_S32 multiCipher_ex_AES_CBC_DEC(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 16;
    HI_U32 u32Testcached = 0;
...
int sample_mutiltcipher(HI_VOID)
{
    ...
    {
        HI_ERR_CIPHER("Multi cipher des ecb encrypt failed.\n");
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Multi cipher des ecb decrypt failed.\n");
        return s32Ret;
    }
#endif
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Multi cipher ex aes cbc encrypt failed.\n");
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_CIPHER("Multi cipher ex aes cbc decrypt failed.\n");
        return s32Ret;
    }
