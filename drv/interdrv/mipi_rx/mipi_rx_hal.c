...
static void write_reg32(const void *addr,
                        unsigned int value,
                        unsigned int mask)
{
    unsigned int t;
...
void mipi_rx_drv_set_work_mode(combo_dev_t devno, input_mode_t input_mode)
{
    ...
    mipi_rx_work_mode_addr = osal_ioremap(MIPI_RX_WORK_MODE_ADDR, (unsigned long)0x4);
...
void mipi_rx_drv_set_mipi_user_dt(combo_dev_t devno, data_type_t input_data_type,
...
void mipi_rx_drv_set_phy_mode(mipi_rx_phy_t phy_id, input_mode_t input_mode, unsigned int lane_bitmap)
{
    ...
    if (lane_bitmap & 0xa) {
        phy_mode_link.bits.phy0_rg_en_clk1 = 1;  // -- 2lane+2lane maybe need test
...
void set_lvds_sync_code(combo_dev_t devno, int n_frame, unsigned int lane_cnt, const short lane_id[LVDS_LANE_NUM],
...
void mipi_rx_drv_set_lvds_nxt_sync_code(combo_dev_t devno, unsigned int lane_cnt,
...
void mipi_rx_drv_set_hs_mode(lane_divide_mode_t mode)
{
    ...
    for (i = 0; i < MIPI_RX_MAX_PHY_NUM; i++) {
        ...
        mipi_rx_set_phy_rg_lp0_mode_en(i, phy_mode[mode][i].phy_rg_lp0_mode_en);
        mipi_rx_set_phy_rg_lp1_mode_en(i, phy_mode[mode][i].phy_rg_lp1_mode_en);
    }
}
...
{
    if (!mipi_rx_regs_va) {
        mipi_rx_regs_va = (mipi_rx_regs_type_t *)osal_ioremap(MIPI_RX_REGS_ADDR, (unsigned int)MIPI_RX_REGS_SIZE);
...
static int mipi_rx_register_irq(void)
{
    ...
    ret = osal_request_irq(mipi_rx_irq_num, mipi_rx_interrupt_route, NULL, "MIPI_RX", mipi_rx_interrupt_route);
    if (ret < 0) {
...
int mipi_rx_drv_init(void)
{
    ...
    mipi_rx_core_reset_addr = osal_ioremap(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
