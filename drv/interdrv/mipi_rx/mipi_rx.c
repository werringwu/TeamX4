...
static int check_lvds_dev_attr(combo_dev_t devno, lvds_dev_attr_t *p_attr)
{
    ...
    if (p_attr->vsync_attr.sync_type < LVDS_VSYNC_NORMAL || p_attr->vsync_attr.sync_type >= LVDS_VSYNC_BUTT) {
...
{
    if ((lane_divide_mode < LANE_DIVIDE_MODE_0) || (lane_divide_mode >= LANE_DIVIDE_MODE_BUTT)) {
    ...
    switch (cmd) {
        case HI_MIPI_SET_DEV_ATTR: {
            ...
            if (ret < 0) {
                ...
                osal_memcpy(&mipi_dev_ctx.combo_dev_attr[devno], pstcombo_dev_attr, sizeof(combo_dev_attr_t));
                osal_spin_unlock(&mipi_ctx_spinlock);
            }
            break;
        }
...
static void proc_show_mipi_device(osal_proc_entry_t *s)
{
    ...
    for (devno = 0; devno < MIPI_RX_MAX_DEV_NUM; devno++) {
        combo_dev_attr_t *pdev_attr = &mipi_dev_ctx.combo_dev_attr[devno];
...
static void proc_show_lvds_lane_detect_info(osal_proc_entry_t *s, combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    for (devno_idx = 0; devno_idx < mipi_cnt; devno_idx++) {
        pstcombo_dev_attr = &mipi_dev_ctx.combo_dev_attr[devno_array[devno_idx]];
...
static void proc_show_mipi_int_err(osal_proc_entry_t *s, combo_dev_t devno_array[], int mipi_cnt)
{
    ...
    osal_seq_printf(s, "%8s%15s%14s%14s%15s\n",
                    "Devno",
                    "DataFifoRdErr", "CmdFifoRdErr", "CmdFifoWrErr", "DataFifoWrErr");
