...
long ssp_lcd_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SSP_LCD_WRITE_CMD16:
            val_16 = *(unsigned int*)(uintptr_t)arg;
...
int hi_ssp_lcd_ota5182_init(void)
{
    ...
    lcd_ota5182_init();
    printk("Kernel: ssp_lcd ota5182 initial ok!\n");

    return 0;

}

void hi_ssp_lcd_ota5182_exit(void)
{
    hi_ssp_disable();
...
#else
static int __init hi_ssp_lcd_ota5182_init(void)
{
    int ret;
