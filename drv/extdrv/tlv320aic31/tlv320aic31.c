...
#ifdef TEST
static int hi_test_i2c_write( unsigned char dev_addr,
                               ...
                               unsigned int data, unsigned int data_byte_num)
{
    ...
    memcpy(&client, tlv_client, sizeof(struct i2c_client));
...
static unsigned char hi_test_i2c_read(unsigned char dev_addr, unsigned int reg_addr,
...
unsigned char tlv320aic31_read(unsigned char dev_addr, unsigned char reg_addr)
{
    return hi_i2c_read(dev_addr, reg_addr, 1, 1);
}

int tlv320aic31_write(unsigned char dev_addr, unsigned char reg_addr, unsigned char data)
{
...
int tlv320aic31_write(unsigned char dev_addr, unsigned char reg_addr, unsigned char value)
{
#if 0
    int ret;
    unsigned char buf[2];
    ...
    return ret;
#endif
    return hi_test_i2c_write(dev_addr, reg_addr, 1, value, 1);
}

int tlv320aic31_read(unsigned char dev_addr, unsigned char reg_addr)
{
#if 0
    int ret_data = 0xFF;
    int ret;
    ...
    return ret_data;
#else
    return hi_test_i2c_read(dev_addr, reg_addr, 1, 1);
#endif
}
...
#endif
{
    ...
    switch (cmd)
    {
        ...
        case SET_CTRL_MODE:
            ...
            {
                /* aic31 mater mode or sample rate is not 44.1K/48k, use external crystal with 12.288MHZ for MCLK as input clk */
            ...
            {
                /* when aic31 slave mode AND sample rate is 44.1k/48k, bclk is for work clk. */
...
#ifdef CONFIG_COMPAT
    .compat_ioctl    = tlv320aic31_ioctl
#endif
};
#else
static struct file_operations_vfs tlv320aic31_fops =
{
    .ioctl  = tlv320aic31_ioctl,
    .open   = tlv320aic31_open,
    .close  = tlv320aic31_close,
};
#endif
...
#ifdef __HuaweiLite__
struct i2c_client tlv320aic31_client_obj;
struct i2c_client * hi_tlv320aic31_i2c_client_init(void)
{
    tlv320aic31_client_obj.addr = IIC_device_addr[0];
    tlv320aic31_client_obj.flags = 0x00;
//    tlv320aic31_client_obj.pinfo = &hi_i2c_data[TLV320AIC31_I2C_ADAPTER_ID];
//  hi_i2c_hw_init(tlv320aic31_client_obj.pinfo);

    client_attach(&tlv320aic31_client_obj, TLV320AIC31_I2C_ADAPTER_ID);

    return &tlv320aic31_client_obj;
}
#endif
...
{
#ifdef __HuaweiLite__
    tlv_client = hi_tlv320aic31_i2c_client_init();
#else
    struct i2c_adapter* i2c_adap;

    i2c_adap = i2c_get_adapter(TLV320AIC31_I2C_ADAPTER_ID);
...
static
#endif
int __init tlv320aic31_init(void)
{
    unsigned int i, ret;
...
#endif
#else
    ret = register_driver("/dev/tlv320aic31", &tlv320aic31_fops, 0666, 0);
    if (ret)
    {
        DPRINTK(0, "could not register tlv320aic31 device");
        return -1;
    ...
    printk("load tlv320aic31.ko for Hi35xx ok!\n");
    return ret;
init_fail:
#ifndef __HuaweiLite__
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
...
#endif
#else
    unregister_driver("/dev/tlv320aic31");
#endif
    i2c_client_exit();

    DPRINTK(1, "deregister tlv320aic31");
    printk("rmmod tlv320aic31.ko for Hi35xx ok!\n");
}
